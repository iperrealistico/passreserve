...
- `2026-06-10 15:07 CEST` Fixed another organizer read-model drift around cancelled and refunded registrations, this time on the organizer-facing registration summaries and related dashboard surfaces. Root cause: the registrations queue was using a local reducer that still counted cancelled entries toward `Partecipanti` and `Da incassare`, while the organizer refund summary view could still show `Rimborso in attesa` if a stale pending refund ledger row coexisted with a now-authoritative `refundedCents` value. Introduced shared helpers for participant counting, outstanding venue-balance calculation, and queue-summary reduction in `lib/passreserve-admin-service.js`; made cancelled/no-show/expired registrations contribute zero to current outstanding venue balances; made fully refunded registrations resolve to `Refund completed` before any stale pending refund hint; and rewired the registrations page summary to use the shared reducer instead of a divergent local implementation. Revalidated with targeted ESLint, focused Vitest suites around admin summaries and refund metadata, and a full green `npm run verify`. Documented in `patch-notes/2026-06-10_15-07-00_organizer-summary-cancel-refund-coherence.md`.
- `2026-06-11 15:12 CEST` Fixed a second refund-lifecycle drift where organizer dashboard and cancelled-registration views could remain stuck on `Refund pending / Rimborso in attesa` even after Stripe had already returned the money. Root cause: the admin read-model itself was already prioritizing `refundedCents` correctly, but in database mode the final `charge.refunded` confirmation still depended on the old full-state webhook mutation path; if that reconciliation did not complete, Passreserve could keep a stale `REFUND/PENDING` ledger row with `registration.refundedCents = 0` even though Stripe had completed the refund. Hardened the refund lifecycle in two layers: added a direct Prisma-based `charge.refunded` path in `lib/passreserve-service.js` so future refund confirmations no longer rely on whole-state replacement, and added a safe organizer-admin reconciliation pass in `lib/passreserve-admin-service.js` that checks pending Stripe refund ids against the connected-account refund state and auto-closes already-succeeded refunds before building dashboard and registrations payloads. Revalidated with focused refund/webhook/admin regressions and a full green `npm run verify`. Documented in `patch-notes/2026-06-11_15-12-00_refund-webhook-reconciliation-and-admin-healing.md`.
...
- `2026-06-10 22:18 CEST` Started the 5-phase registration-language rollout and completed Phase 1 (model and rules foundation) by adding organizer/event-level `registrationLanguagePromptEnabled` config with safe defaults and inherit semantics, introducing a shared `passreserve-registration-language` helper for `it/en` normalization plus prompt resolution metadata, exposing the resolved prompt state in organizer/public event read models, updating file-state + seed + organizer-provisioning defaults, applying the new Prisma migration on the canonical production schema, and closing the phase with targeted locale/resolver regressions plus a full green `npm run verify`. Patch note: `patch-notes/2026-06-10_22-18-00_registration-language-foundation.md`.
- `2026-06-11 10:07 CEST` Completed Phase 2 (organizer/event configuration UI) of the registration-language rollout by adding the new shared `RegistrationLanguagePromptEditor` to organizer Settings and event editing, wiring the form state through organizer admin actions into `updateOrganizerSettings()` and `saveOrganizerEvent()`, persisting both organizer defaults and nullable event-level overrides in database/file-state runtimes, and adding focused regressions for organizer default saves plus event override/set-clear behavior before another full green `npm run verify`. Patch note: `patch-notes/2026-06-11_10-07-00_registration-language-admin-controls.md`.
- `2026-06-11 10:26 CEST` Completed Phase 3 (public booking flow integration) of the registration-language rollout by making the public registration page accept an explicit `bookingLocale` override, adding the live booking-language selector to step 1 when the organizer/event config asks for it, preserving the in-progress registration draft across locale switches, persisting the chosen language into `registrationLocale`, carrying that locale through the pending redirect, and making the token-based confirm/confirmed/payment pages resolve header/footer/event locale from the stored registration language without changing email templates yet. Revalidated with focused registration-language/registration-flow regressions plus a full green `npm run verify`. Patch note: `patch-notes/2026-06-11_10-26-00_registration-language-public-booking-flow.md`.
- `2026-06-11 10:46 CEST` Completed Phase 4 (attendee email localization) of the registration-language rollout by extending `EmailTemplate` with optional `subject / preview / bodyHtml` translation maps, seeding retrocompatible IT/EN variants for attendee-facing transactional templates, backfilling the canonical production schema with a new Prisma migration that preserves existing English content while adding Italian defaults, making attendee email sends resolve localized template variants from `registration.registrationLocale` with legacy fallback, localizing dynamic attendee email placeholders such as registration source notes, refund-state copy, event titles, date labels, time ranges, venue names, and reminder fallback copy, and upgrading the platform email console so admins can edit default, Italian, and English variants side by side. Revalidated with targeted ESLint, focused Vitest suites for localization/admin persistence/password reset/registration flows, a full green `npm run verify`, successful `npx prisma migrate deploy` against the canonical `passreserve` schema, and a clean `npx prisma migrate status`. Patch note: `patch-notes/2026-06-11_10-46-20_registration-language-attendee-email-localization.md`.
- `2026-06-11 10:57 CEST` Completed Phase 5 (hardening, regression coverage, and rollout closeout) of the registration-language rollout by tightening regression coverage around locale-aware attendee email delivery in every live branch that now depends on `registrationLocale`: public pending-confirmation sends, direct-confirm attendee confirmations, organizer manual-registration confirmation/payment-link sends, reminder deliveries, and organizer-triggered cancellation emails. No new runtime behavior was introduced in this pass; instead, the phase formalized the rollout safety net and revalidated the entire stack with the full `npm run verify` gate so the language selector + localized attendee emails now have end-to-end regression coverage across the active booking lifecycle. Patch note: `patch-notes/2026-06-11_10-57-00_registration-language-hardening-and-rollout-closeout.md`.
- `2026-06-11 15:48 CEST` Added structured event-level refund policy disclosure and mandatory customer acceptance to the public booking flow without altering organizer accounts, Stripe refund logic, or existing registration lifecycles. Introduced the new `RefundPolicyType` model plus registration-side `refundPolicyAcceptedAt` and `refundPolicySnapshot`, exposed localized refund-policy summaries/details in the public event payload, added organizer event controls for `Refundable / Non-refundable / Refundable with conditions`, surfaced the policy card on public event detail pages, required acceptance before creating a hold or direct-confirm registration, and persisted the accepted policy snapshot for later audit/display. Also updated file-state + seed migration behavior so legacy events with existing cancellation text default safely to `REFUNDABLE_WITH_CONDITIONS`, revalidated with focused Vitest suites and a full green `npm run verify`, and applied the new Prisma migration on the canonical `passreserve` schema while confirming the live Sillico event `divini-sapori` now carries `REFUNDABLE_WITH_CONDITIONS` with its detailed organizer policy text. Patch note: `patch-notes/2026-06-11_15-48-00_refund-policy-disclosure-and-booking-acceptance.md`.
- `2026-06-11 18:18 CEST` Fixed the cookie banner positioning regression introduced by the privacy-compliance rollout: the banner CSS had conflicting `fixed` and `relative` positioning, which caused it to anchor to the page flow and render half outside the viewport until users scrolled. Removed the conflicting relative layout behavior, constrained the width to the viewport, kept the banner fixed above page content on every breakpoint, and revalidated with a fresh production build before republishing. Patch note: `patch-notes/2026-06-11_18-18-00_cookie-banner-fixed-viewport-positioning.md`.
- `2026-06-11 18:27 CEST` Reworked the `Current choice / Scelta attuale` badge used inside booking-language and confirmation-mode cards so it no longer inherits the multiline descriptive span styling. Introduced a dedicated compact badge treatment with no wrapping, tighter vertical rhythm, and a smaller aspect ratio while preserving the rest of the card layout and the live booking-language flow unchanged. Revalidated with a fresh production build before republishing. Patch note: `patch-notes/2026-06-11_18-27-00_confirmation-mode-badge-compact-rework.md`.
- `2026-06-11 18:31 CEST` Removed the redundant ticket-mix helper sentence from the public booking flow (`You can mix different tickets in the same purchase / Puoi combinare piu ticket nello stesso acquisto`) because the ticket-step UI already makes this capability self-evident. This was a copy-only cleanup with no booking-logic changes, revalidated through a fresh production build before republishing. Patch note: `patch-notes/2026-06-11_18-31-00_remove-redundant-ticket-mix-hint.md`.
- `2026-06-11 18:35 CEST` Fixed the attendee-card heading layout in the public registration flow by reusing the existing `registration-choice-copy` stack for `Partecipante 1 / Partecipante principale` instead of leaving the subtitle inline. This restores proper spacing and vertical hierarchy without changing any attendee/questionnaire logic, and was revalidated through a fresh production build before republishing. Patch note: `patch-notes/2026-06-11_18-35-00_attendee-heading-spacing-fix.md`.
- `2026-06-11 18:39 CEST` Removed the direct-confirm informational notice box from the final review step of the public booking flow because it exposed an internal/system-oriented explanation that added no decision value for guests. This was a copy-only cleanup of the `DIRECT_CONFIRM` branch, with no changes to confirmation mode logic, email behavior, or validation, and it was revalidated through a fresh production build before republishing. Patch note: `patch-notes/2026-06-11_18-39-00_remove-direct-confirm-user-notice.md`.
- `2026-06-11 18:44 CEST` Fixed the public results-card location rendering so city and region no longer collapse visually into strings like `BolzanoDolomites` or `Pieve FoscianaToscana`, by adding an explicit separator and dedicated inline spacing in `/events`. In the same pass, removed the six stock seed organizers from the canonical production database (`alpine-trail-lab`, `lago-studio-pass`, `officina-gravel-house`, `atelier-del-gusto`, `comune-aperto`, `studio-movimento-sud`) with their cascaded sample events, while explicitly preserving the real `sillico` organizer in `Pieve Fosciana, Toscana` and all of its events. Revalidated with a fresh production build and post-delete production checks before republishing. Patch note: `patch-notes/2026-06-11_18-44-00_results-location-spacing-and-template-organizer-cleanup.md`.
- `2026-06-11 18:49 CEST` Removed the preselected black-border highlight from the public event detail occurrence cards so the date list no longer visually implies that one specific evening is already chosen for the guest. This change only affected the event-detail date list presentation (`agenda-item` cards) and left event routing, selected-occurrence summary logic, and registration links untouched. Revalidated through a fresh production build before republishing. Patch note: `patch-notes/2026-06-11_18-49-00_remove-event-occurrence-highlight.md`.
- `2026-06-11 19:36 CEST` Removed the organizer-access example copy from the public `/organizer-access` page by deleting the explanatory example sentence under the CTA row and clearing the `example` placeholder from the slug input. This keeps the organizer-login entrypoint focused on the actual slug field without extra demo text, and did not alter organizer routing, auth actions, or platform-admin access. Revalidated through a fresh production build before republishing. Patch note: `patch-notes/2026-06-11_19-36-00_remove-organizer-access-example-copy.md`.
- `2026-06-11 19:43 CEST` Simplified the public `/events` location label by removing the visual middle-dot separator between city and region, so listings now read as plain spaced location text like `Pieve Fosciana Toscana`. This only affected the discovery-card location row and left event discovery queries, organizer links, and card layout behavior otherwise unchanged. Revalidated through a fresh production build before republishing. Patch note: `patch-notes/2026-06-11_19-43-00_simplify-events-location-spacing.md`.
- `2026-06-11 19:52 CEST` Refined the public `/events` discovery cards and filters in one pass: removed the empty leading whitespace before the price by dropping the unused empty organizer-tagline slot, introduced a larger dedicated price badge paired with the collection label, and added country/region/city filters with a default scoped view of `Italy / Tuscany`. The discovery service now returns sanitized geographic filter metadata, preserves filter state in search chips and reset links, and keeps `/events` limited only when both search and geographic filters are unset. Revalidated through a fresh production build before republishing. Patch note: `patch-notes/2026-06-11_19-52-00_events-price-badge-and-geographic-filters.md`.
- `2026-06-11 20:01 CEST` Removed the two non-essential pill badges from the organizer-page hero (`venue title` and default collection label), leaving only the location kicker, organizer title, summary copy, and the primary `View calendar / Vedi calendario` CTA. This was a presentation-only cleanup of the public organizer page and did not alter organizer data, agenda rendering, or event registration links. Revalidated through a fresh production build before republishing. Patch note: `patch-notes/2026-06-11_20-01-00_remove-organizer-hero-pills.md`.
- `2026-06-11 20:07 CEST` Simplified the public organizer-page agenda so it now shows only the first upcoming date for each public event type instead of listing every occurrence of the same event repeatedly. Event detail pages still retain the full occurrence list for that event type, so this change only reduces duplication on the organizer overview page and leaves registration routing untouched. Revalidated through a fresh production build before republishing. Patch note: `patch-notes/2026-06-11_20-07-00_organizer-agenda-first-occurrence-per-event.md`.
- `2026-06-11 20:15 CEST` Removed the operator identity line (`Leonardo Fiori · VAT/P. IVA ...`) from both the public site footer and the admin footer so those details now live only inside the Privacy, Cookie Policy, and Terms documents instead of the persistent site chrome. This was a footer-only presentation cleanup; legal content remains unchanged in the dedicated documents. Revalidated through a fresh production build before republishing. Patch note: `patch-notes/2026-06-11_20-15-00_remove-operator-identity-from-footers.md`.
- `2026-06-11 20:22 CEST` Made event duration optional across the organizer event editor and public event payload by dropping the legacy `180-minute` fallback, allowing blank duration saves to persist as `null`, and only rendering the duration pill when an event actually defines one. Existing events keep their current values, so `Divini & Sapori` still shows `3h 00m` while future events can omit duration entirely. Revalidated with a Prisma schema migration, targeted admin-event regression coverage, a fresh production build, and live checks before republishing. Patch note: `patch-notes/2026-06-11_20-22-00_make-event-duration-optional.md`.
- `2026-06-11 20:31 CEST` Rebalanced the desktop footer navigation offset so the `Home / Events / About / Organizer access` line now sits on the same vertical band as the footer summary line (`Minimal event registration software for local organizers.`) instead of hanging noticeably lower. This was a pure spacing adjustment in the shared public footer CSS, revalidated through a fresh production build before republishing. Patch note: `patch-notes/2026-06-11_20-31-00_footer-nav-summary-baseline-tune.md`.
- `2026-06-11 20:36 CEST` Replaced the homepage organizer hero headline to remove the explicit Airbnb reference while keeping the same punch and length in both languages. The new copy is `Operate events with precision and clarity.` / `Gestisci eventi con precisione e chiarezza.` with no structural layout changes. Revalidated through a fresh production build before republishing. Patch note: `patch-notes/2026-06-11_20-36-00_home-hero-organizer-headline-refresh.md`.
- `2026-06-11 20:44 CEST` Simplified the public organizer-admin login page to a single-panel flow: the sign-in form is shown by default, the reset-password form is opened through a dedicated button on that same panel, and the reset form now includes a `Back to sign in / Torna al login` button instead of living beside a second always-visible card. The auth and password-reset actions remain unchanged apart from keeping the reset panel open after a reset-link request. Revalidated through a fresh production build and live route checks before republishing. Patch note: `patch-notes/2026-06-11_20-44-00_single-panel-organizer-login-and-reset.md`.
- `2026-06-11 20:49 CEST` Removed the remaining stock copy block from the default organizer-admin login panel so it now opens directly on the organizer name and credential form without the generic `Organizer admin sign in`, descriptive paragraph, or `Events / Schedule / Registrations` pills. The reset-password panel keeps its explanatory copy, while the sign-in experience is now quieter and more task-focused. Revalidated through a fresh production build and live route checks before republishing. Patch note: `patch-notes/2026-06-11_20-49-00_remove-login-panel-stock-copy.md`.
- `2026-06-11 20:58 CEST` Reworked the homepage organizer-access request consent control so the terms/privacy acceptance checkbox now renders as a compact inline checkbox on the left of the legal sentence instead of as a large stacked block. The legal wording and required validation remain unchanged; this was a presentation-only cleanup to make the form feel normal and reduce wasted space. Revalidated through a fresh production build and live route checks before republishing. Patch note: `patch-notes/2026-06-11_20-58-00_compact-home-organizer-consent-checkbox.md`.
- `2026-06-11 21:05 CEST` Added a compact icon to the homepage `What is Passreserve?` story link so the CTA feels more intentional and visible without changing the copy or layout structure. The link now uses a small inline information mark on the left, with no behavior changes beyond the visual affordance. Revalidated through a fresh production build and live homepage check before republishing. Patch note: `patch-notes/2026-06-11_21-05-00_home-story-link-icon.md`.
- `2026-06-11 22:02 CEST` Rebuilt the desktop public footer alignment structurally instead of relying on manual top padding. The footer main row now uses a desktop grid where the brand title occupies the first row, the summary line and footer nav share the second row, and the language switcher spans the right edge. Also removed the desktop-only vertical padding from footer nav links so the nav text can sit on the same visual band as the summary copy. This replaces the earlier fragile `padding-top` approximation with a stable row-based layout. Revalidated through a fresh production build and live browser measurement checks before republishing. Patch note: `patch-notes/2026-06-11_22-02-00_footer-structural-row-alignment.md`.
- `2026-06-13 12:00 CEST` Recovered the live Sillico production dataset after the overnight incident that replaced the real organizer/event state with demo seed data and 404s. Created a safety backup of the corrupted canonical schema as `recovery_backup_20260613_115143`, rebuilt the live `sillico` organizer/admin/event/ticket/occurrence state directly in the canonical production database, restored the essential `SiteSettings`, `AboutPageContent`, and `EmailTemplate` rows needed by the live runtime, and reintroduced the missing `RegistrationAttendee_registrationId_fkey` constraint so attendee rows cannot drift orphaned from registrations again. In parallel, hardened `lib/passreserve-state.js` so protected Vercel production database runtimes now fail closed instead of auto-seeding demo state when the canonical database is incomplete, and fixed the nested `RegistrationItem` Prisma write shape that could previously abort full-state rewrites and trigger the fallback path. Revalidated with full `npm run verify`, production DB inspection, public-route checks for `/sillico`, `/sillico/events/divini-sapori`, and organizer-auth verification for `leterredelmoro@gmail.com`. Patch note: `patch-notes/2026-06-13_12-00-32_phase-13_production-sillico-restore-and-demo-seed-guard.md`.
- `2026-06-13 12:38 CEST` Investigated a remaining live organizer-login failure on `https://passreserve.com/sillico/admin/login` after the Sillico data restore. Production Vercel runtime logs showed intermittent GET failures with `Error: Cookies can only be ...`, which traced back to render-time session cleanup inside the shared organizer admin layout path: invalid organizer/platform session helpers were still trying to `save()` or `destroy()` the session during normal page rendering, which Vercel/Next forbids outside Server Actions or Route Handlers. Removed cookie mutation from the read-only validation helpers and from the `require*Session()` redirect path in `lib/passreserve-auth.js`, keeping the security behavior of rejecting stale sessions while making the login page render-safe even when a browser arrives with an old/invalid cookie. Revalidated with `npm run lint`, `npm run test`, and `npm run build` before republishing. Patch note: `patch-notes/2026-06-13_12-38-00_organizer-login-render-safe-session-validation.md`.
- `2026-06-13 13:48 CEST` Hardened Passreserve against future production storage drift by introducing an explicit storage policy layer, making Vercel production fail closed instead of silently falling back to the file store, wrapping non-mutator full-state Prisma rewrites in their own advisory-lock transaction, and asserting snapshot integrity before database-wide rewrites or restores. Added workspace-local backup tooling with `npm run ops:backup`, `npm run ops:backup:weekly`, and `npm run ops:restore`, created the first verified local snapshot under `.ops/backups/passreserve`, confirmed the restore script refuses to target the primary database without explicit override, documented the new ops flow in the runtime handoff docs, and created the weekly local Codex automation `passreserve-weekly-local-backup` for Sunday 03:30 Europe/Rome runs. Revalidated with a full green `npm run verify`. Patch note: `patch-notes/2026-06-13_13-48-00_storage-fail-closed-and-local-backup-automation.md`.
- `2026-06-13 14:28 CEST` Removed the redundant secondary `Events` button from the homepage `Search events` card, leaving only the primary `Browse events / Esplora eventi` submit action so the attendee entrypoint is simpler without altering search behavior, routing, or any database-backed flows. Revalidated with `npm run lint` and `npm run build` before republishing. Patch note: `patch-notes/2026-06-13_14-28-05_remove-home-search-events-secondary-button.md`.

...
- `2026-04-27 18:19 CEST` Rebalanced the organizer card again from screenshot review by restoring the same two-row rhythm as the attendee module: a full-width dark faux-input line now carries the access context, while the split `Request access / Open organizer access` control drops to action-row scale with a capped width so it no longer outweighs the left card; re-ran `npm run verify` successfully before publishing the follow-up.

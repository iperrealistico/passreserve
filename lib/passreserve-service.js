  ...
  if (getStorageMode() === "database") {
    ...
    const entries = organizers.flatMap((organizer) =>
      organizer.events.map((event) => {
        const nextOccurrence = event.occurrences.find(
          (occurrence) => new Date(occurrence.startsAt).getTime() > now
        );
        ...
        return {
          ...
          eventHref: `/${organizer.slug}/events/${event.slug}`,
          registrationHref:
            nextOccurrence
              ? `/${organizer.slug}/events/${event.slug}/register?occurrenceId=${nextOccurrence.id}`
              : `/${organizer.slug}/events/${event.slug}`,
          collectionLabel: formatCollectionLabel(event.prepayPercentage),
          priceLabel: formatCurrencyFromCents(event.basePriceCents),
          searchText: [
            organizer.name,

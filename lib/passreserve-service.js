...
function buildConfirmedView(state, registration) {
  ...
  return {
    ...
    paymentProvider: {
      label: registration.onlineAmountCents > 0 ? "Stripe Checkout" : "Pay at the event"
    },
    registrationCode: registration.registrationCode,
...
export async function processStripeWebhook(event) {
  ...
  return mutatePersistentState(async (draft) => {
    ...
    const registrationCode = session.client_reference_id || session.metadata?.registration_code;
    ...
    await finalizeOnlinePayment(draft, registration, {
      ...
      stripePaymentIntentId:
        typeof session.payment_intent === "string"
          ? session.payment_intent
          : session.payment_intent?.id || null,
      note: "Stripe webhook confirmed checkout completion.",
      metadata: {
        type: event.type
      }
    });

...
} from "./passreserve-public-read-model.js";
import {
...
} from "./passreserve-public-cache.js";
import {
  loadFileBackedState,
  loadPersistentState,
...
const PUBLIC_REGISTRATION_TRANSACTION_OPTIONS = {
  maxWait: 10_000,
  timeout: 20_000
};

function buildPrismaPublicStateSnapshot({
  organizer = null,
  organizerAdmins = [],
  eventRecord = null,
  occurrences = [],
...
async function readPrismaPublicEventRegistrationState(prisma, slug, eventSlug) {
  ...
  const eventRecord = await prisma.eventType.findFirst({
    where: {
  ...
  if (!eventRecord) {
    return buildPrismaPublicStateSnapshot({
      organizer
    });
  }

  const [organizerAdmins, ticketCategories, occurrences, registrations, siteSettings] =
    await Promise.all([
...
async function readPrismaRegistrationStateByHoldToken(prisma, holdToken) {
  ...
  const [organizer, eventRecord, occurrence, organizerAdmins, ticketCategories, siteSettings] =
    await Promise.all([
      ...
      prisma.eventType.findUnique({
        where: {
          id: registration.eventTypeId
        }
      }),
      prisma.eventOccurrence.findUnique({
        where: {
          id: registration.occurrenceId
        }
      }),
      prisma.organizerAdminUser.findMany({
        where: {
...
async function readPrismaRegistrationStateByPaymentToken(prisma, paymentToken) {
  const registration = await prisma.registration.findFirst({
    where: {
      paymentToken
    },
...
async function readPrismaRegistrationStateByConfirmationToken(prisma, confirmationToken) {
  const registration = await prisma.registration.findFirst({
    where: {
      confirmationToken
    },
  ...
  const [organizer, eventRecord, occurrence, organizerAdmins, ticketCategories, siteSettings, payments] =
    await Promise.all([
      prisma.organizer.findUnique({
        where: {
          id: registration.organizerId
...
const loadPublicOrganizerStateLegacyBySlug = cache(async function loadPublicOrganizerStateLegacyBySlug(slug) {
...
const loadPublicOrganizerStateV2BySlug = cache(
  async function loadPublicOrganizerStateV2BySlug(slug) {
    if (getStorageMode() !== "database") {
      return null;
    }
    ...
    try {
      const prisma = getPrismaClient();
      if (isPublicDataCacheEnabled()) {
        const publicSlugs = await readPublicOrganizerSlugIndexWithCache(
          async () => {
            ...
            return organizers
              .map((organizer) => organizer.publicSlug)
              .filter(Boolean);
          }
        );
      ...
      const content = await readPublicOrganizerContentWithCache(
        slug,
        (publicSlug) => readPrismaPublicOrganizerContentV2(prisma, publicSlug)
      );
      ...
      logDatabaseFallback(
        "[passreserve-service] public organizer read model v2 unavailable, falling back to legacy public query",
        error
      );

      return loadPublicOrganizerStateLegacyBySlug(slug);
    }
  }
);

function loadPublicOrganizerStateBySlug(slug) {
  return getPublicReadModelVersion() === PUBLIC_READ_MODEL_VERSION.V2
    ? loadPublicOrganizerStateV2BySlug(slug)
    : loadPublicOrganizerStateLegacyBySlug(slug);
}
  ...
  return {
    ...
    event: {
      ...
      collectionLabel: formatCollectionLabel(
        occurrenceRecord.prepayPercentage ?? eventRecord.prepayPercentage,
        occurrenceRecord.paymentSplitMode ?? eventRecord.paymentSplitMode,
        ticketCategoryRecord.fixedOnlineAmountCents
      )
    },
    occurrence: {
...
function formatCollectionLabel(
  prepayPercentage,
  paymentSplitMode = PAYMENT_SPLIT_MODE.PERCENTAGE,
  fixedOnlineAmountCents = null
) {
  if (
    normalizePaymentSplitMode(paymentSplitMode) === PAYMENT_SPLIT_MODE.FIXED_AMOUNTS
  ) {
    return Number.isInteger(fixedOnlineAmountCents)
      ? `${formatCurrencyFromCents(fixedOnlineAmountCents)} online`
      : "Exact amount online";
  }
...
function buildOccurrenceCapacitySummary(state, occurrence, event) {
  ...
  return summarizeOccurrenceCapacity({
    registrations,
    occurrence,
    event
  });
...
function buildTicketCategoryView(category, paymentSource, locale = "en", quantity = 1) {
  const payment = calculatePaymentBreakdown({
    unitPrice: category.unitPriceCents / 100,
    quantity,
    prepayPercentage: paymentSource?.prepayPercentage || 0,
    paymentSplitMode: paymentSource?.paymentSplitMode,
...
function buildOccurrenceView(state, organizer, event, occurrence, ticketCategories, locale = "en") {
  ...
  const paymentSplitMode = normalizePaymentSplitMode(occurrence.paymentSplitMode);
  const ticketCategoryViews = getSafeEntries(ticketCategories).map((category) =>
    buildTicketCategoryView(category, occurrence, locale)
  );
  ...
  return {
    ...
    collectionLabel: formatCollectionLabel(
      ...
      paymentSplitMode,
      defaultTicketCategory?.fixedOnlineAmountCents
    ),
    capacity,
...
function buildEventView(state, organizer, event, locale = "en") {
  ...
  const ticketCategoryViews = ticketCategories.map((category) =>
    buildTicketCategoryView(category, event, locale)
  );
  const defaultTicketCategory = getDefaultTicketCategory(ticketCategoryViews);
  const payment = calculatePaymentBreakdown({
    ...
    paymentSplitMode: event.paymentSplitMode,
    fixedOnlineAmountCents: defaultTicketCategory?.fixedOnlineAmountCents,
    fixedDueAtEventCents: defaultTicketCategory?.fixedDueAtEventCents
  });
...
async function runNonBlockingPrismaCheckoutSideEffect(step, operation, context = {}) {
  try {
    return await operation();
  } catch (error) {
    console.error(`[passreserve-service] ${step}`, {
      ...context,
...
function buildCheckoutRedirectHref(slug, eventSlug, paymentToken, sessionUrl = null) {
  return sessionUrl || buildPaymentPreviewHref(slug, eventSlug, paymentToken);
}

async function persistPrismaCheckoutLaunchSideEffects({
  prisma,
  session,
  registration,
  organizer,
  ...
  eventSlug,
  source,
  organizerAlert = false
}) {
  const redirectHref = buildCheckoutRedirectHref(
    slug,
    eventSlug,
  ...
  if (session?.sessionId) {
    await runNonBlockingPrismaCheckoutSideEffect(
      "failed to persist Stripe checkout session record",
      () =>
  ...
  await runNonBlockingPrismaCheckoutSideEffect(
    "failed to persist payment checkout audit log",
    () =>
      createPrismaPaymentCheckoutAuditLog(prisma, {
        organizerId: organizer?.id || null,
        registrationId: registration.id,
        registrationCode: registration.registrationCode,
  ...
  if (organizerAlert && emailContext) {
    await runNonBlockingPrismaCheckoutSideEffect(
      "failed to send organizer new registration alert after checkout handoff",
      () =>
        sendPrismaOrganizerNewRegistrationAlert(
          prisma,
          registration,
          emailContext,
          platformReplyEmail
...
async function persistPrismaCheckoutFallbackSideEffects({
  prisma,
  registration,
  organizer,
  emailContext = null,
  ...
  const redirectHref = buildPaymentPreviewHref(slug, eventSlug, registration.paymentToken);

  await runNonBlockingPrismaCheckoutSideEffect(
    "failed to persist payment checkout failure audit log",
    () =>
      createPrismaPaymentCheckoutAuditLog(prisma, {
        organizerId: organizer?.id || null,
        registrationId: registration.id,
  ...
  if (organizerAlert && emailContext) {
    await runNonBlockingPrismaCheckoutSideEffect(
      "failed to send organizer new registration alert after checkout fallback",
      () =>
        sendPrismaOrganizerNewRegistrationAlert(
          prisma,
          registration,
...
const DISCOVERY_QUERY_MAX_LENGTH = 160;
const DISCOVERY_RESULT_LIMIT = 250;

export function normalizePublicDiscoveryQuery(query = "") {
  const normalized = normalizeText(
    typeof query === "string" ? query : ""
  ).replace(/\s+/g, " ");

  return {
    normalized: normalized.length <= DISCOVERY_QUERY_MAX_LENGTH ? normalized : "",
    rejected: normalized.length > DISCOVERY_QUERY_MAX_LENGTH
  };
}

async function readPrismaPublicDiscoveryEntries(locale) {
  const events = await getPrismaClient().eventType.findMany({
    where: {
      visibility: "PUBLIC",
      organizer: {
        status: "ACTIVE",
...
async function readFilePublicDiscoveryEntries(locale) {
  const state = await loadPersistentState();
  ...
  return organizers.flatMap((organizer) =>
...
function buildDiscoveryResultSet(entries, normalizedQuery, requestedFilters) {
  ...
  const databaseCompatibleEntries = normalizedQuery
    ? filteredEntries.filter(
        (entry) =>
          !Array.isArray(entry.databaseSearchFields) ||
          entry.databaseSearchFields.some((field) => field.includes(normalizedQuery))
      )
    : filteredEntries;
  const rankedEntries = rankDiscoveryEntries(
    databaseCompatibleEntries,
    normalizedQuery
  );
  ...
  return {
    results:
      normalizedQuery || hasActiveFilters
        ? rankedEntries.slice(0, DISCOVERY_RESULT_LIMIT)
        : rankedEntries.slice(0, 8),
...
export async function getDiscoveryResults(query = "", locale = "en", filters = {}) {
  ...
  const normalizedQuery = normalizePublicDiscoveryQuery(query);

  if (normalizedQuery.rejected) {
    return buildDiscoveryResultSet([], "", requestedFilters);
  }

  if (getStorageMode() === "database") {
    try {
      const entries = await readPublicDiscoveryCatalogWithCache(
        normalizedLocale,
        readPrismaPublicDiscoveryEntries
      );

      return buildDiscoveryResultSet(
        entries,
      ...
      logDatabaseFallback(
        "[passreserve-service] discovery database catalog unavailable, falling back to file state",
        error
      );
    }
  }

  const entries = await readFilePublicDiscoveryEntries(normalizedLocale);
  return buildDiscoveryResultSet(
    entries,
    normalizedQuery.normalized.toLowerCase(),
    requestedFilters
...
async function createRegistrationHoldInDatabase(payload) {
  ...
  const outcome = await prisma.$transaction(async (tx) => {
    ...
    const state = await readPrismaPublicEventRegistrationState(
      tx,
      payload.slug,
      payload.eventSlug
    );
    ...
    const buildResult = prepareRegistrationBuild({
      ...
      paymentSplitMode:
        context.selectedOccurrence.paymentSplitMode ?? context.event.paymentSplitMode,
      nowIso: now
    });
  ...
  if (outcome.registration.holdToken === null) {
    if (outcome.registration.onlineAmountCents > 0) {
      ...
      try {
        ...
        return {
          ok: true,
          redirectHref: await persistPrismaCheckoutFallbackSideEffects({
            prisma,
            registration: outcome.registration,
            organizer: outcome.organizer,
            emailContext: outcome.emailContext,
            platformReplyEmail: outcome.platformReplyEmail,
            slug: payload.slug,
      ...
      return {
        ok: true,
        redirectHref: await persistPrismaCheckoutLaunchSideEffects({
          prisma,
          session,
          registration: outcome.registration,
          organizer: outcome.organizer,
...
async function confirmRegistrationHoldInDatabase(payload) {
  ...
  const outcome = await prisma.$transaction(async (tx) => {
    await tx.$executeRawUnsafe(`SELECT pg_advisory_xact_lock(${SYSTEM_LOCK_ID})`);
    const state = await readPrismaRegistrationStateByHoldToken(tx, payload.holdToken);
...
export async function getConfirmedRegistrationView(slug, eventSlug, confirmationToken) {
  if (getStorageMode() === "database") {
    try {
      const prisma = getPrismaClient();
      const state = await readPrismaRegistrationStateByConfirmationToken(prisma, confirmationToken);
      const registration = state ? getRegistrationByConfirmationToken(state, confirmationToken) : null;
      ...
      console.error("[passreserve-service] getConfirmedRegistrationView failed in database mode", error);

      return {
        state: "error",
        title: "This confirmation could not be loaded right now.",
...
async function resumeRegistrationPaymentInDatabase(input) {
  const prisma = getPrismaClient();
  const state = await readPrismaRegistrationStateByPaymentToken(prisma, input.paymentToken);
  ...
  try {
    ...
    console.error("[passreserve-service] failed to reopen Stripe Checkout session", error);
  ...
  if (session.sessionId) {
    ...
    await runNonBlockingPrismaCheckoutSideEffect(
      "failed to persist reopened checkout handoff",
      () =>
  ...
  return {
    ok: true,
    redirectHref: buildCheckoutRedirectHref(
      getOrganizerPublicSlug(organizer) || organizer.slug,
      eventRecord.slug,
      registration.paymentToken,
      session.url
    )
...
async function resolveSuccessfulRegistrationConfirmationInDatabase(input) {
  const prisma = getPrismaClient();
  const initialState = await readPrismaRegistrationStateByPaymentToken(prisma, input.paymentToken);
  const initialRegistration = initialState
    ? getRegistrationByPaymentToken(initialState, input.paymentToken)
    : null;
  ...
  const outcome = await prisma.$transaction(async (tx) => {
    await tx.$executeRawUnsafe(`SELECT pg_advisory_xact_lock(${SYSTEM_LOCK_ID})`);
    const state = await readPrismaRegistrationStateByPaymentToken(tx, input.paymentToken);
    const registration = state ? getRegistrationByPaymentToken(state, input.paymentToken) : null;

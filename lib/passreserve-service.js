...
const CONFIRMATION_RULES = [
  {
    field: "termsAccepted",
    label: "Terms, privacy, and organizer policy",
    detail:
      "The attendee acknowledges the site privacy notice, accepts the site terms, and confirms the organizer notes, venue guidance, and published policies."
  },
  {
...
function validateDirectConfirmationRequirements(payload) {
  const missing = [];
  ...
  if (payload.responsibilityAccepted !== "yes") {
    missing.push("responsibilityAccepted");
  }
  ...
  return {
    ok: false,
    message:
      "Confirm the organizer terms and attendee readiness before completing the registration.",
...
async function createRegistrationHoldInDatabase(payload) {
  ...
  const outcome = await prisma.$transaction(async (tx) => {
    ...
    if (registrationConfirmationMode === REGISTRATION_CONFIRMATION_MODE.DIRECT_CONFIRM) {
      const confirmationValidation = validateDirectConfirmationRequirements(payload);
...
function buildPendingConfirmationEmailReplacements(context, baseUrl, registration) {
  return {
    ...
    "{{registration_source_note}}": getRegistrationSourceNote(registration),

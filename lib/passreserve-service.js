...
function sumRegistrationItemQuantity(items = []) {
...
function buildRegistrationPaymentTotals(items = []) {
...
function normalizeRegistrationAttendees(attendees = [], nowIso = new Date().toISOString(), options = {}) {
  const collectDietaryInfo = options.collectDietaryInfo !== false;
  ...
  return attendees.map((attendee, index) => ({
    ...
    dietaryFlags: collectDietaryInfo
      ? [...new Set((attendee.dietaryFlags || []).filter((flag) => dietaryFlagIds.includes(flag)))]
      : [],
    dietaryOther: collectDietaryInfo ? normalizeText(attendee.dietaryOther) : "",
    createdAt: nowIso,
    updatedAt: nowIso
  }));
}
...
function buildRequestedItemsByTicket(items = []) {
  return new Map(
    getSafeEntries(items).map((item) => [item.ticketCategoryId, Number(item.quantity || 0)])
  );
}

function countAttendeesByTicket(attendees = []) {
...
function buildRegistrationLineItems(requestedItems, ticketCategoryMap, prepayPercentage, nowIso) {
...
export async function createRegistrationHold(input) {
  ...
  if (requestedQuantity !== payload.attendees.length) {
    return {
      ok: false,
  ...
  return mutatePersistentState(async (draft) => {
    ...
    const selectedTicketCategoryMap = new Map(
      selectedTicketCategories.map((category) => [category.id, getTicketCategoryById(draft, category.id)])
    );
    const requestedItemsByTicket = buildRequestedItemsByTicket(requestedItems);
    const collectDietaryInfo = context.eventRecord.collectDietaryInfo !== false;

    if (
      requestedItems.some(
        (item) =>
          !selectedTicketCategoryMap.has(item.ticketCategoryId) ||
          !selectedTicketCategoryMap.get(item.ticketCategoryId)
      )
    ) {
      return {
        ok: false,
    ...
    const attendeeCountsByTicket = countAttendeesByTicket(attendees);
    ...
    const registration = {
      ...
      ticketCategoryId: leadItem?.ticketCategoryId || requestedItems[0]?.ticketCategoryId,
...
export async function confirmRegistrationHold(input) {
  ...
  return mutatePersistentState(async (draft) => {
    ...
    if (
      !organizer ||
      !matchesOrganizerPublicSlug(organizer, parsed.data.slug) ||
      !eventRecord ||

...
import {
  getPrismaClient,
  logDatabaseFallback,
  repairPassreserveSchema
} from "./passreserve-prisma.js";
...
const loadPublicOrganizerStateBySlug = cache(async function loadPublicOrganizerStateBySlug(slug) {
  ...
  try {
    ...
    if (!(await repairPassreserveSchema(error))) {
      logDatabaseFallback(
...
function buildEventView(state, organizer, event, locale = "en") {
  ...
  return {
    ...
    interestHref: `mailto:${organizer.publicEmail}?subject=${encodeURIComponent(

...
export const organizerOperationsPhase = {
  label: "Phase 10",
  title: "Organizer operations dashboard, calendar, registrations, and payments UI",
  summary:
    "Passreserve.com now turns organizer admin into a live operations surface with registration queues, organizer-local calendar views, payment visibility, and reconciliation workflows."
};

export const organizerOperationsGuidance = [
  {
    title: "Registration and revenue metrics replace booking counts",
    detail:
      "Organizer dashboards now track confirmations, pending payment pressure, online collection, and balance still due at the venue."
  },
  {
    title: "Calendar stays occurrence-first",
    detail:
      "Daily operations stay anchored to event occurrences, but each day now surfaces attendee counts, payment-state pressure, and capacity watch signals."
  },
  {
    title: "Reconciliation remains practical",
    detail:
      "Organizer actions cover confirmation, cancellation, no-show handling, online payment confirmation, and venue-balance reconciliation without leaving the admin shell."
  }
];
...
function createRecord({
  ...
  const rawRecord = {
    ...
    attendeeName,
    attendeeEmail: `${emailStem || "attendee"}@passreserve-demo.test`,
    ...
    items: [
      {
        ...
        detail: `Calendar grouping, daily metrics, and queue timestamps all resolve in ${timeZone}. This snapshot is anchored to ${referenceLabel}.`
      },
      {
        title: "Occurrence rendering",
        detail:
          "Occurrence cards show local start and end times rather than server-local windows, which keeps daily operations aligned with the organizer's venue schedule."
      },
      {
  ...
  return updatePayment(
    {
      ...record,
      status: nextStatus,
      note: "Venue balance was reconciled inside the organizer operations queue."
    },
    {
  ...
  return updatePayment(
    {
      ...record,
      status: "CANCELLED",
      note: "Registration was cancelled from the organizer queue."
    },
    {
...
export function applyRegistrationOperation(records, recordId, actionId) {
  let message = "No organizer action was applied.";
  ...
  const nextRecords = records.map((record) => {
    ...
    switch (actionId) {
      case "confirm":
        message = `${record.registrationCode} confirmed and moved into the next organizer state.`;
      ...
      case "cancel":
        message = `${record.registrationCode} cancelled and removed from active operations queues.`;

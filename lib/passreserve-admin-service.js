...
function parseRequiredIsoDate(value, label) {
...
export async function saveOrganizerOccurrence(slug, input, actorId = null) {
  if (getStorageMode() === "database") {
    ...
    return prisma.$transaction(async (tx) => {
      ...
      const startsAt = parseRequiredIsoDate(input.startsAt, "Start time");
      const endsAt = parseRequiredIsoDate(input.endsAt, "End time");

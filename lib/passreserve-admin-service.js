...
export async function getOrganizerShell(slug) {
  if (getStorageMode() === "database") {
    ...
    const [registrationRows, totalUpcomingOccurrences] = await Promise.all([
      prisma.registration.findMany({
        where: {
          organizerId: organizer.id
        },
    ...
    const activeCount = registrationRows.filter((registration) => {
      if (["CANCELLED", "NO_SHOW"].includes(registration.status)) {
        return false;
      }

      if (registration.status === "PENDING_CONFIRM" || registration.status === "PENDING_PAYMENT") {
        return !registration.expiresAt || registration.expiresAt.getTime() > now.getTime();
      }

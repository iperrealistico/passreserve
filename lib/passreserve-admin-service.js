...
import {
  PAYMENT_SPLIT_MODE,
  normalizePaymentSplitMode
} from "./passreserve-domain.js";
...
import { normalizeEmailTemplateTranslations } from "./passreserve-email-templates.js";
import {
...
import { getRefundPolicyTypeMeta, normalizeRefundPolicyType } from "./passreserve-refund-policy.js";
...
function getPendingStripeRefundId(payment) {
  const metadata =
    payment?.metadata && typeof payment.metadata === "object" && !Array.isArray(payment.metadata)
      ? payment.metadata
      : null;

  return metadata?.stripeRefundId ? String(metadata.stripeRefundId) : null;
}

function buildReconciledRefundPaymentMetadata(payment, refund, now, extraMetadata = {}) {
  const baseMetadata =
    payment?.metadata && typeof payment.metadata === "object" && !Array.isArray(payment.metadata)
      ? payment.metadata
      : {};
  ...
  return {
    ...baseMetadata,
    ...extraMetadata,
    stripeRefundId: refund?.refundId || getPendingStripeRefundId(payment) || null,
    stripeRefundStatus: refund?.status || baseMetadata.stripeRefundStatus || null,
    stripeFailureReason: refund?.failureReason || baseMetadata.stripeFailureReason || null,
    stripeAccountId: refund?.stripeAccountId || payment?.stripeAccountId || baseMetadata.stripeAccountId || null,
    stripePaymentIntentId:
      refund?.paymentIntentId ||
      payment?.stripePaymentIntentId ||
      baseMetadata.stripePaymentIntentId ||
      null,
    reconciledAt: now
  };
}

export async function reconcilePendingStripeRefundsForOrganizer(organizer) {
  if (!organizer?.id) {
    return;
  }

  const prisma = getPrismaClient();
  const pendingRefundPayments = await prisma.registrationPayment.findMany({
    where: {
      registration: {
  ...
  for (const payment of pendingRefundPayments) {
    const stripeRefundId = getPendingStripeRefundId(payment);
    ...
    try {
      const refund = await retrieveStripeRefund(stripeRefundId, payment.stripeAccountId);
      ...
      if (refund.status === "succeeded") {
        ...
        await prisma.$transaction(async (tx) => {
          const currentPayment = await tx.registrationPayment.findUnique({
            where: {
              id: payment.id
            }
          });
          ...
          const nextRefundedCents = Math.max(
            currentRegistration.refundedCents || 0,
            refund.amountCents || 0,
            currentPayment.amountCents || 0
          );

          if (nextRefundedCents > (currentRegistration.refundedCents || 0)) {
          ...
          await tx.registrationPayment.update({
            ...
            data: {
              status: "REFUNDED",
              amountCents: refund.amountCents || currentPayment.amountCents,
              note: "Stripe refund reconciled from connected-account state.",
              metadata: buildReconciledRefundPaymentMetadata(currentPayment, refund, nowIso, {
          ...
          await tx.auditLog.create({
            data: {
              id: createToken(),
              actorType: "STRIPE",
              organizerId: currentRegistration.organizerId,
              registrationId: currentRegistration.id,
              eventType: "stripe_refund_confirmed",
              entityType: "registration_payment",
              entityId: currentRegistration.id,
              message: `Stripe refund reconciled for ${currentRegistration.registrationCode || currentRegistration.id}.`,
              metadata: {
                type: "refund.reconciled",
                stripeRefundId,
                stripeRefundStatus: refund.status,
                refundDeltaCents: refund.amountCents || currentPayment.amountCents || 0,
                matchedPendingRefund: true,
                reconciledFrom: "organizer_admin_load"
              },
              createdAt: now
            }
          });
      ...
      if (refund.status === "failed" || refund.status === "canceled") {
        ...
        await prisma.$transaction(async (tx) => {
          ...
          await tx.registrationPayment.update({
            ...
            data: {
              status: "FAILED",
              note: "Stripe refund did not complete and needs organizer follow-up.",
              metadata: buildReconciledRefundPaymentMetadata(currentPayment, refund, nowIso, {
                reconciliationSource: "organizer_admin_load",
                errorMessage: refund.failureReason || null
              }),
              occurredAt: now
            }
          });
          ...
          await tx.auditLog.create({
            data: {
              id: createToken(),
              actorType: "STRIPE",
              organizerId: payment.registration.organizerId,
              registrationId: payment.registration.id,
              eventType: "organizer_refund_request_failed",
              entityType: "registration_payment",
              entityId: payment.registration.id,
              message: `Stripe refund follow-up is required for ${payment.registration.registrationCode || payment.registration.id}.`,
              metadata: {
                action: "organizer_cancel",
                stripeRefundId,
                stripeRefundStatus: refund.status,
      ...
      console.error(
        "[passreserve-admin-service] pending Stripe refund reconciliation failed",
        {
...
function isRegistrationOperationallyActive(registration) {
...
export function getRegistrationParticipantCount(registration) {
  ...
  if (Array.isArray(registration?.ticketItems) && registration.ticketItems.length > 0) {
    return registration.ticketItems.reduce((sum, item) => sum + Number(item.quantity || 0), 0);
  }

  return Number(registration?.quantity || 1);
}

export function getRegistrationOutstandingVenueBalanceCents(registration) {
  if (!isRegistrationOperationallyActive(registration)) {
    return 0;
  }
...
export function summarizeOrganizerRegistrationList(registrations, options = {}) {
  const countHistoricalParticipants = Boolean(options.countHistoricalParticipants);

  return (Array.isArray(registrations) ? registrations : []).reduce(
    (accumulator, registration) => {
      const participantCount = getRegistrationParticipantCount(registration);
      const restrictionsCount = Number(registration?.dietary?.participantsWithRestrictions || 0);
      const isCurrentRegistration = countHistoricalParticipants
        ? true
        : registration?.operationallyActive ?? isRegistrationOperationallyActive(registration);

      accumulator.registrationCount += 1;
      if (isCurrentRegistration) {
        accumulator.participantCount += participantCount;
        accumulator.restrictionsCount += restrictionsCount;
        accumulator.dueAtVenueCents += getRegistrationOutstandingVenueBalanceCents(registration);
      }
      ...
      if (registration?.refundSummary?.status === "READY") {
        ...
        accumulator.refundReadyCents +=
          Number(registration.refundSummary.refundableOnlineAmountCents || 0);
      }

      if (registration?.refundSummary?.status === "PENDING") {
        accumulator.refundPendingCount += 1;
        accumulator.refundPendingCents += Number(registration.refundSummary.pendingRefundCents || 0);
      }
...
const loadOrganizerAdminStateBySlug = cache(async function loadOrganizerAdminStateBySlug(slug) {
  ...
  try {
    ...
    await reconcilePendingStripeRefundsForOrganizer(organizer);
...
function buildOrganizerAdminEventRecord(state, organizer, event) {
  const resolvedRegistrationQuestionnaireConfig = resolveRegistrationQuestionnaireConfig(
    organizer,
    event
  );
  const resolvedRegistrationLanguagePromptEnabled = resolveRegistrationLanguagePromptEnabled(
    organizer,
    event
  );
  const resolvedRegistrationConfirmationMode = resolveRegistrationConfirmationMode(organizer, event);
  ...
  return {
    ...event,
    paymentSplitMode: normalizePaymentSplitMode(event.paymentSplitMode),
    refundPolicyType: normalizeRefundPolicyType(event.refundPolicyType, null),
    refundPolicyTypeMeta: getRefundPolicyTypeMeta(event.refundPolicyType),
    visibility: normalizeAdminEventVisibility(event.visibility),
    gallery: normalizeAdminGalleryItems(event.gallery),
    collectDietaryInfo: shouldCollectDietaryFromQuestionnaire(resolvedRegistrationQuestionnaireConfig),
    registrationQuestionnaireConfig: event.registrationQuestionnaireConfig || null,
    resolvedRegistrationQuestionnaireConfig,
    hasRegistrationQuestionnaireOverride: Boolean(event.registrationQuestionnaireConfig),
    registrationLanguagePromptEnabled:
      typeof event.registrationLanguagePromptEnabled === "boolean"
        ? event.registrationLanguagePromptEnabled
        : null,
    resolvedRegistrationLanguagePromptEnabled,
    registrationLanguagePromptMeta: getRegistrationLanguagePromptMeta(
      resolvedRegistrationLanguagePromptEnabled
    ),
    hasRegistrationLanguagePromptOverride:
      typeof event.registrationLanguagePromptEnabled === "boolean",
    registrationConfirmationMode: event.registrationConfirmationMode || null,
    resolvedRegistrationConfirmationMode,
    registrationConfirmationModeMeta: getRegistrationConfirmationModeMeta(
      resolvedRegistrationConfirmationMode
    ),
    hasRegistrationConfirmationOverride: Boolean(event.registrationConfirmationMode),
    ticketCategories,
...
function getOrganizerSummary(state, organizer) {
  ...
  const activeRegistrations = registrations.filter(isRegistrationOperationallyActive);
  const totals = getRegistrationTotals(registrations);
  ...
  return {
    ...
    grossOnlineCollected: totals.grossOnlineCollected,
    grossOnlineCollectedLabel: totals.grossOnlineCollectedLabel,
    refundedOnline: totals.refundedOnline,
    refundedOnlineLabel: totals.refundedOnlineLabel,
...
function getRegistrationTotals(registrations) {
  const grossOnlineCollected = registrations.reduce(
    (sum, registration) => sum + (registration.onlineCollectedCents || 0),
    0
  );
  const refundedOnline = registrations.reduce(
    (sum, registration) => sum + (registration.refundedCents || 0),
    0
  );
  ...
  const dueAtEvent = registrations.reduce((sum, registration) => {
    return sum + getRegistrationOutstandingVenueBalanceCents(registration);
  }, 0);

  return {
    grossOnlineCollected,
    grossOnlineCollectedLabel: formatCurrencyFromCents(grossOnlineCollected),
    refundedOnline,
    refundedOnlineLabel: formatCurrencyFromCents(refundedOnline),
    onlineCollected,
...
function buildOccurrenceLabelForOrganizer(organizer, occurrence, locale = "en") {
  return formatDateLabel(occurrence.startsAt, organizer.timeZone, locale);
}
...
function buildCancellationEmailReplacements(
  ...
  return {
    ...
    "{{event_name}}": getLocalizedText(event, "title", locale) || event.title,
    "{{occurrence_label}}": buildOccurrenceLabelForOrganizer(organizer, occurrence, locale),
...
async function sendStateRegistrationCancellationEmail(
  ...
  return sendStateTemplateEmail(state, {
    ...
    locale: registration.registrationLocale,
...
async function sendStateOccurrenceCancellationEmail(
  ...
  return sendStateTemplateEmail(state, {
    ...
    replacements: {
      "{{event_name}}":
        getLocalizedText(event, "title", registration.registrationLocale || "en") || event.title,
      "{{occurrence_label}}": buildOccurrenceLabelForOrganizer(
        organizer,
        occurrence,
...
function normalizeTicketCatalogInput(value, fallbackBasePriceCents = 0) {
  ...
  const normalized = parsed
    ...
    .map((entry, index) => {
      ...
      const fixedOnlineAmountCents = parseOptionalTicketAmountCents(
        entry.fixedOnlineAmountEuros,
        entry.fixedOnlineAmountCents
      );
      const fixedDueAtEventCents = parseOptionalTicketAmountCents(
        entry.fixedDueAtEventEuros,
        entry.fixedDueAtEventCents
      );
      ...
      return {
        ...
        included,
        unitPriceCents,
        fixedOnlineAmountCents,
        fixedDueAtEventCents,
    ...
    return [
      {
        ...
        fixedOnlineAmountCents: null,
        fixedDueAtEventCents: null,
        isDefault: true,
        isActive: true,
        sortOrder: 0
      }
...
function parseOptionalTicketAmountCents(eurosValue, centsValue) {
  const normalizedEuros = normalizeText(eurosValue).replace(",", ".");

  if (normalizedEuros) {
    const parsedEuros = Number(normalizedEuros);
    return Number.isFinite(parsedEuros) && parsedEuros >= 0
      ? Math.round(parsedEuros * 100)
      : null;
  }

  if (centsValue === "" || centsValue == null) {
    return null;
  }

  const parsedCents = Number(centsValue);
  return Number.isFinite(parsedCents) && parsedCents >= 0 ? Math.round(parsedCents) : null;
}

function assertValidFixedTicketAmounts(ticketCatalog, paymentSplitMode) {
  if (normalizePaymentSplitMode(paymentSplitMode) !== PAYMENT_SPLIT_MODE.FIXED_AMOUNTS) {
    return;
  }

  for (const ticket of ticketCatalog.filter((entry) => entry.isActive !== false)) {
    if (
      !Number.isInteger(ticket.fixedOnlineAmountCents) ||
      !Number.isInteger(ticket.fixedDueAtEventCents)
    ) {
      throw new Error(`Add both exact payment amounts for the ${ticket.name} ticket.`);
    }

    if (
      ticket.fixedOnlineAmountCents + ticket.fixedDueAtEventCents !==
      ticket.unitPriceCents
    ) {
      throw new Error(
        `For the ${ticket.name} ticket, the online and at-event amounts must equal the ticket price.`
      );
    }
  }
}
...
function buildOrganizerAdminRecord(state, registration, locale = "en") {
  ...
  return {
    ...
    dueAtEventOpenLabel: formatCurrencyFromCents(
      getRegistrationOutstandingVenueBalanceCents(registration)
    ),
...
export async function markAdminLogin(scope, userId) {
  if (getStorageMode() === "database") {
    ...
    if (scope === "platform") {
      await prisma.$transaction(async (tx) => {
        const admin = await tx.platformAdminUser.findUnique({
          where: {
            id: userId
          },
          select: {
        ...
        await tx.platformAdminUser.updateMany({
          where: {
            id: userId
          },
          data: {
        ...
        if (admin) {
          await tx.auditLog.create({
            data: {
              ...
              organizerId: null,
              registrationId: null,
              eventType: "platform_admin_login_success",
              entityType: "platform_admin",
              entityId: admin.id,
              message: `Platform admin login recorded for ${admin.email}.`,
              metadata: {
    ...
    await prisma.$transaction(async (tx) => {
      const admin = await tx.organizerAdminUser.findUnique({
        where: {
          id: userId
        },
        select: {
      ...
      await tx.organizerAdminUser.updateMany({
        where: {
          id: userId
        },
        data: {
      ...
      if (admin?.organizer) {
        await tx.auditLog.create({
          data: {
            ...
            actorId: admin.id,
            organizerId: admin.organizer.id,
            registrationId: null,
            eventType: "organizer_admin_login_success",
            entityType: "organizer_admin",
            entityId: admin.id,
            message: `Organizer admin login recorded for ${admin.email}.`,
            metadata: {
              adminName: admin.name || null,
              organizerSlug: admin.organizer.slug,
              organizerName: admin.organizer.name
            }
          }
        });
      }
  ...
  await mutatePersistentState(async (draft) => {
    ...
    if (admin) {
      ...
      draft.auditLogs.unshift({
        ...
        actorType: scope === "platform" ? "PLATFORM_ADMIN" : "ORGANIZER_ADMIN",
        actorId: admin.id,
        organizerId:
          scope === "platform"
            ? null
            : draft.organizers.find((entry) => entry.id === admin.organizerId)?.id || null,
        registrationId: null,
        eventType:
          scope === "platform"
        ...
        entityId: admin.id,
        message: `${
          scope === "platform" ? "Platform" : "Organizer"
        } admin login recorded for ${admin.email}.`,
        metadata:
          scope === "platform"
            ? {
                adminName: admin.name || null
            ...
            : {
                adminName: admin.name || null,
                organizerSlug:
                  draft.organizers.find((entry) => entry.id === admin.organizerId)?.slug || null,
                organizerName:
                  draft.organizers.find((entry) => entry.id === admin.organizerId)?.name || null
              }
      });
    }
  });
...
const getPlatformOverviewCached = cache(async function getPlatformOverviewCached() {
  ...
  const activeRegistrations = state.registrations.filter(
    (registration) => isRegistrationOperationallyActive(registration)
  );
  const totals = getRegistrationTotals(state.registrations);
      ...
      checks: [
        {
          ...
          statusTone: storage.isHealthy ? "public" : "capacity-watch",
  ...
  if (getStorageMode() === "database") {
    ...
    return organizers.map((organizer) => {
      const activeRegistrations = organizer.registrations.filter((registration) =>
        isRegistrationOperationallyActive(registration)
      );
...
export const getOrganizerLoginShell = cache(async function getOrganizerLoginShell(slug) {
  if (getStorageMode() === "database") {
    try {
      const organizer = await getPrismaClient().organizer.findUnique({
        where: {
          slug
        },
        select: {
      ...
      return organizer
        ? {
            organizer: serializeDatabaseValue(organizer)
          }
        : null;
    } catch (error) {
      logDatabaseFallback(
        "[passreserve-admin-service] organizer login shell unavailable in database, falling back to file state",
        error
      );
    }
  }
      ...
      occurrences: getOrganizerEvents(state, organizer.id)
        .flatMap((event) =>
          getEventOccurrences(state, event.id).map((occurrence) => ({
            ...
            usesOnlinePayments: isOccurrenceUsingOnlinePayments(
              occurrence,
              getTicketCategoriesForEvent(state, event.id)
            ),
    ...
    return {
      ...
      registrations: getOrganizerRegistrations(state, organizer.id).map((registration) => {
        ...
        return {
          ...
          dueAtEventOpenCents: getRegistrationOutstandingVenueBalanceCents(registration),
...
export async function updateOrganizerSettings(slug, input, actorId = null) {
  if (getStorageMode() === "database") {
    ...
    const registrationQuestionnaireConfig = input.registrationQuestionnaireConfig
      ? normalizeRegistrationQuestionnaireConfig(input.registrationQuestionnaireConfig)
      : currentOrganizer.registrationQuestionnaireConfig
        ? normalizeRegistrationQuestionnaireConfig(currentOrganizer.registrationQuestionnaireConfig)
        : null;
    const registrationLanguagePromptEnabled = normalizeRegistrationLanguagePromptEnabledInput(
      input.registrationLanguagePromptEnabled,
      currentOrganizer.registrationLanguagePromptEnabled ?? true
    );
    const registrationConfirmationMode = normalizeRegistrationConfirmationMode(
      input.registrationConfirmationMode,
      currentOrganizer.registrationConfirmationMode || "EMAIL_LINK_REQUIRED"
    );
    const now = new Date();
    ...
    await prisma.auditLog.create({
      data: {
        ...
        metadata: {
          ...
          hasRegistrationQuestionnaireConfig: Boolean(organizer.registrationQuestionnaireConfig),
          registrationLanguagePromptEnabled: organizer.registrationLanguagePromptEnabled,
          registrationConfirmationMode: organizer.registrationConfirmationMode,
  ...
  return mutatePersistentState(async (draft) => {
    ...
    const registrationLanguagePromptEnabled = normalizeRegistrationLanguagePromptEnabledInput(
      input.registrationLanguagePromptEnabled,
      organizer.registrationLanguagePromptEnabled ?? true
    );
    const registrationConfirmationMode = normalizeRegistrationConfirmationMode(
      input.registrationConfirmationMode,
      organizer.registrationConfirmationMode || "EMAIL_LINK_REQUIRED"
    );
    ...
    organizer.registrationQuestionnaireConfig = registrationQuestionnaireConfig;
    organizer.registrationLanguagePromptEnabled = registrationLanguagePromptEnabled;
    organizer.registrationConfirmationMode = registrationConfirmationMode;
...
export async function updateEmailTemplate(input, actorId = null) {
  return mutatePersistentState(async (draft) => {
    ...
    template.subjectTranslations = normalizeEmailTemplateTranslations(
      input.subjectTranslations
    );
    template.previewTranslations = normalizeEmailTemplateTranslations(
      input.previewTranslations
    );
    template.bodyHtmlTranslations = normalizeEmailTemplateTranslations(
      input.bodyHtmlTranslations
    );
...
export async function saveOrganizerEvent(slug, input, actorId = null) {
  if (getStorageMode() === "database") {
    ...
    return prisma.$transaction(async (tx) => {
      ...
      const paymentSplitMode = normalizePaymentSplitMode(input.paymentSplitMode);
      const paymentSplitModeChanged = Boolean(
        existingEvent &&
          normalizePaymentSplitMode(existingEvent.paymentSplitMode) !== paymentSplitMode
      );
      const prepayPercentage = Math.max(
        0,
        Math.min(100, Number(input.prepayPercentage || 0))
      );
      assertValidFixedTicketAmounts(ticketCatalog, paymentSplitMode);
      ...
      const organizerQuestionnaireConfig = organizer.registrationQuestionnaireConfig
        ? normalizeRegistrationQuestionnaireConfig(organizer.registrationQuestionnaireConfig)
        : null;
      ...
      const organizerRegistrationLanguagePromptEnabled =
        resolveRegistrationLanguagePromptEnabled(organizer, null);
      const nextRegistrationQuestionnaireConfig = input.registrationQuestionnaireConfig
        ? normalizeRegistrationQuestionnaireConfig(input.registrationQuestionnaireConfig, {
            baseConfig: organizerQuestionnaireConfig || undefined
          })
        : null;
      const nextRegistrationLanguagePromptEnabled =
        normalizeText(input.registrationLanguagePromptEnabled) !== ""
          ? normalizeRegistrationLanguagePromptEnabledInput(
              input.registrationLanguagePromptEnabled,
              organizerRegistrationLanguagePromptEnabled
            )
          : null;
      const nextRegistrationConfirmationMode = input.registrationConfirmationMode
        ? normalizeRegistrationConfirmationMode(
            input.registrationConfirmationMode,
            organizerRegistrationConfirmationMode
          )
        : null;
      const nextRefundPolicyType = normalizeRefundPolicyType(input.refundPolicyType, null);
      const resolvedRegistrationQuestionnaireConfig = resolveRegistrationQuestionnaireConfig(
        {
          registrationQuestionnaireConfig: organizerQuestionnaireConfig
        },
        {
          registrationQuestionnaireConfig: nextRegistrationQuestionnaireConfig,
          collectDietaryInfo: existingEvent?.collectDietaryInfo ?? true
        }
      );
      ...
      const eventData = {
        ...
        durationMinutes: parseOptionalDurationMinutes(input.durationMinutes),
        venueTitle:
        ...
        prepayPercentage,
        paymentSplitMode,
        collectDietaryInfo: shouldCollectDietaryFromQuestionnaire(
          resolvedRegistrationQuestionnaireConfig
        ),
        registrationQuestionnaireConfig: nextRegistrationQuestionnaireConfig,
        registrationLanguagePromptEnabled: nextRegistrationLanguagePromptEnabled,
        registrationConfirmationMode: nextRegistrationConfirmationMode,
        salesWindowStartsAt,
        salesWindowEndsAt,
        ...
        refundPolicyType: nextRefundPolicyType,
      ...
      for (const ticket of ticketCatalog) {
        ...
        const ticketData = {
          ...
          fixedOnlineAmountCents: ticket.fixedOnlineAmountCents,
          fixedDueAtEventCents: ticket.fixedDueAtEventCents,
  ...
  return mutatePersistentState(async (draft) => {
    ...
    if (paymentSplitModeChanged) {
      const registeredOccurrenceIds = new Set(
        draft.registrations.map((registration) => registration.occurrenceId)
      );

      for (const occurrence of draft.occurrences) {
        if (
          occurrence.eventTypeId === event.id &&
          !registeredOccurrenceIds.has(occurrence.id)
        ) {
          occurrence.paymentSplitMode = paymentSplitMode;
          occurrence.prepayPercentage = prepayPercentage;
          occurrence.updatedAt = ticketNow;
        }
      }
    }
...
export async function saveOrganizerOccurrence(slug, input, actorId = null) {
  ...
  if (getStorageMode() === "database") {
    ...
    const outcome = await prisma.$transaction(async (tx) => {
      ...
      const resolvedPaymentSplitMode = normalizePaymentSplitMode(
        input.paymentSplitMode || event.paymentSplitMode
      );
      const eventTicketCategories = await tx.ticketCategory.findMany({
        where: {
          eventTypeId: event.id,
          isActive: true
        }
      });
      assertValidFixedTicketAmounts(eventTicketCategories, resolvedPaymentSplitMode);
      const nextOccurrence = {
        priceCents: Math.round(event.basePriceCents),
        prepayPercentage: Math.max(0, Math.min(100, resolvedPrepayPercentage)),
        paymentSplitMode: resolvedPaymentSplitMode,
      ...
      if (
        ...
        isOccurrenceUsingOnlinePayments(nextOccurrence, eventTicketCategories) &&
        !billingGate.enabled
      ) {
      ...
      const occurrenceData = {
        ...
        paymentSplitMode: nextOccurrence.paymentSplitMode,
        venueTitle:
  ...
  const outcome = await mutatePersistentState(async (draft) => {
    ...
    const eventTicketCategories = draft.ticketCategories.filter(
      (entry) => entry.eventTypeId === event.id && entry.isActive !== false
    );
    assertValidFixedTicketAmounts(eventTicketCategories, resolvedPaymentSplitMode);

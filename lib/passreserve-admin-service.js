...
import { getSharedMailboxConsole, isMailboxInboundConfigured } from "./passreserve-mailbox.js";
import {
...
const getPlatformOverviewCached = cache(async function getPlatformOverviewCached() {
  if (getStorageMode() === "database") {
    ...
    const unreadMailboxCount = unreadThreads.reduce(
    ...
    return {
      ...
      attentionQueue: [
        ...
        {
          title: `${unreadMailboxCount} unread mailbox messages`,
          detail: "Use the shared inbox to reply from contact@ and keep operational email threads inside Passreserve.",
          href: "/admin/emails?tab=mailbox",
          cta: "Open mailbox"
        },
        {
      ...
      releaseTracks: [
        ...
        },
        {
          ...
          detail: "Registration, payment, mailbox, and audit activity are stored durably."
        }
      ]
    };
  }
  ...
  const unreadMailboxCount = state.mailboxThreads.reduce(
    (sum, thread) => sum + Number(thread.unreadCount || 0),
    0
  );
...
export async function getPlatformHealth() {
  ...
  if (getStorageMode() === "database") {
    ...
    const inboundConfigured = Boolean(
      process.env.RESEND_API_KEY?.trim() && process.env.RESEND_WEBHOOK_SECRET?.trim()
    );
    const [organizerCount, eventCount, occurrenceCount, registrationCount, recentFailures] =
      await Promise.all([
        prisma.organizer.count(),
        prisma.eventType.count(),
        prisma.eventOccurrence.count(),
    ...
    return {
      ...
      checks: [
        ...
        {
          title: "Inbound email",
          statusLabel: inboundConfigured ? "Configured" : "Needs webhook setup",
          statusTone: inboundConfigured ? "public" : "capacity-watch",
          detail:
            inboundConfigured
              ? "Resend inbound webhooks can now populate the shared mailbox for contact@ replies."
              : "Add the Resend webhook secret and webhook endpoint to activate the shared mailbox receiver."
        }
      ],
      email: {
        outboundModeLabel: emailConfigured ? "Resend configured" : "Log only",
        outboundConfigured: emailConfigured,
        inboundSupported: inboundConfigured,
      ...
      risks: [
        ...
        {
          title: "Shared inbox depends on the Resend receiving domain",
          detail:
            "Make sure the `contact@` inbox points at Resend and the webhook endpoint is registered before relying on the in-app mailbox."
        }
      ]
    };
  }
  ...
  const state = await loadPersistentState();
  const inboundConfigured = isMailboxInboundConfigured(state);
...
export async function getPlatformEmailConsole(options = {}) {
  const mailbox = await getSharedMailboxConsole({
  ...
  if (getStorageMode() === "database") {
    ...
    return {
      ...
      registrations: getOrganizerRegistrations(state, organizer.id).map((registration) => {
        ...
        return {
          ...buildOrganizerAdminRecord(state, registration, locale),
          actions: getRegistrationActionOptions(registration),

...
export const ORGANIZER_ADMIN_TOUR_DEFAULT_STORAGE_SEED = "base";
...
function normalizeStorageSeed(seed) {
  return typeof seed === "string" && seed.trim()
    ? seed.trim()
    : ORGANIZER_ADMIN_TOUR_DEFAULT_STORAGE_SEED;
}

export function buildOrganizerAdminTourStorageKey({
  slug,
  seed = ORGANIZER_ADMIN_TOUR_DEFAULT_STORAGE_SEED
}) {
  return `${ORGANIZER_ADMIN_TOUR_STORAGE_KEY}:${slug}:${normalizeStorageSeed(seed)}`;
}

export function getOrganizerAdminTourStorageKeyCandidates({
  slug,
  seed = ORGANIZER_ADMIN_TOUR_DEFAULT_STORAGE_SEED
}) {
  const normalizedSeed = normalizeStorageSeed(seed);
  const currentKey = buildOrganizerAdminTourStorageKey({
    slug,
    seed: normalizedSeed
  });

  if (normalizedSeed !== ORGANIZER_ADMIN_TOUR_DEFAULT_STORAGE_SEED) {
    return [currentKey];
  }

  return [currentKey, `${ORGANIZER_ADMIN_TOUR_STORAGE_KEY}:${slug}`, ORGANIZER_ADMIN_TOUR_STORAGE_KEY];
}
...
function buildEnglishSetupSteps(slug) {
  return [
    ...
    },
    {
      ...
      description:
        "Fill in the slug, category, visibility, duration if relevant, and sales rules. This is the foundation for the first event container.",
      side: "right",
      align: "start"
    },
    {

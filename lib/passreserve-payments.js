...
function normalizeStripeMetadata(metadata = null) {
...
function normalizeStripeRefundReason(reason) {
  return ["duplicate", "fraudulent", "requested_by_customer"].includes(reason)
    ? reason
    : undefined;
}

export function buildStripeRefundIdempotencyKey({
  ...
  return [
    ...
    "refund",
    action,
    registrationId || paymentIntentId || chargeId || "unscoped",
...
export function buildStripeRefundRequest({
  amountCents = null,
  chargeId = null,
  idempotencyKey = null,
  metadata = null,
  ...
  if (!paymentIntentId && !chargeId) {
    throw new Error("Stripe refunds require either a payment_intent or charge reference.");
  }

  if (amountCents != null && (!Number.isFinite(normalizedAmount) || normalizedAmount <= 0)) {
    throw new Error("Stripe refund amounts must be positive integers in the smallest currency unit.");
  }
  ...
  const params = {
    ...
    ...(normalizeStripeMetadata(metadata)
      ? {
          metadata: normalizeStripeMetadata(metadata)
        }
      : {}),
    ...(normalizeStripeRefundReason(reason)
      ? {
...
export function summarizeStripeRefund(refund, options = {}) {
  if (!refund) {
    return null;
  }

  const currency = String(refund.currency || options.currency || "eur").toUpperCase();
  const amountCents = refund.amount ?? options.amountCents ?? 0;
  ...
  return {
    ...
    amountCents,
    amountLabel: formatCurrencyFromMinorUnits(amountCents, currency),
    currency,
    chargeId: typeof refund.charge === "string" ? refund.charge : refund.charge?.id || null,
    paymentIntentId:
      typeof refund.payment_intent === "string"
        ? refund.payment_intent
        : refund.payment_intent?.id || options.paymentIntentId || null,
...
export async function createStripeCheckoutSession({
  ...
  const environment = getStripeEnvironmentState();
  const resolvedBaseUrl = environment.baseUrl;
...
export async function createStripeRefund(
  ...
  if (!stripeAccountId) {
    throw new Error("Stripe refunds require the organizer connected account id.");
  }
  ...
  if (!stripe) {
    return {
      ...
      refundId: null,
      status: "preview",
      amountCents: amountCents != null ? Math.round(Number(amountCents)) : null,
      amountLabel:
        amountCents != null
          ? formatCurrencyFromMinorUnits(Math.round(Number(amountCents)), environment.defaultCurrency)
          : null,
      currency: environment.defaultCurrency.toUpperCase(),
      chargeId: chargeId || null,
      paymentIntentId: paymentIntentId || null,
      stripeAccountId,
      idempotencyKey: idempotencyKey || null,
      metadata: normalizeStripeMetadata(metadata) || {},
      reason: normalizeStripeRefundReason(reason) || null,
      failureReason: null,
      createdAt: null
    };
  }

  const refund = await stripe.refunds.create(request.params, request.requestOptions);

  return summarizeStripeRefund(refund, {
    amountCents,
    idempotencyKey,
    mode: "live",
    paymentIntentId,
...
export async function retrieveStripeRefund(refundId, stripeAccountId = null, options = {}) {
  if (!refundId) {
    return null;
  }

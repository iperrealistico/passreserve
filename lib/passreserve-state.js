...
const PRODUCTION_DATABASE_STATE_ERROR_MESSAGE =
  "Passreserve production database state is incomplete. Refusing to seed demo data.";

class ProductionDatabaseStateError extends Error {
  constructor(message = PRODUCTION_DATABASE_STATE_ERROR_MESSAGE) {
    super(message);
    this.name = "ProductionDatabaseStateError";
  }
}
...
function assertSafeToSeedDatabaseState(state) {
  if (!hasStateData(state) && isProtectedProductionRuntime()) {
    throw new ProductionDatabaseStateError();
  }
}

export function summarizePersistentStateSnapshot(state) {
  return {
    organizerCount: Array.isArray(state?.organizers) ? state.organizers.length : 0,
    organizerAdminCount: Array.isArray(state?.organizerAdmins) ? state.organizerAdmins.length : 0,
    platformAdminCount: Array.isArray(state?.platformAdmins) ? state.platformAdmins.length : 0,
    joinRequestCount: Array.isArray(state?.joinRequests) ? state.joinRequests.length : 0,
    eventCount: Array.isArray(state?.events) ? state.events.length : 0,
    ticketCategoryCount: Array.isArray(state?.ticketCategories)
      ? state.ticketCategories.length
      : 0,
    occurrenceCount: Array.isArray(state?.occurrences) ? state.occurrences.length : 0,
    registrationCount: Array.isArray(state?.registrations) ? state.registrations.length : 0,
    paymentCount: Array.isArray(state?.payments) ? state.payments.length : 0,
    mailboxThreadCount: Array.isArray(state?.mailboxThreads) ? state.mailboxThreads.length : 0,
    mailboxMessageCount: Array.isArray(state?.mailboxMessages)
      ? state.mailboxMessages.length
      : 0,
    mailboxAttachmentCount: Array.isArray(state?.mailboxAttachments)
      ? state.mailboxAttachments.length
      : 0,
    emailDeliveryCount: Array.isArray(state?.emailDeliveries) ? state.emailDeliveries.length : 0,
    emailTemplateCount: Array.isArray(state?.emailTemplates) ? state.emailTemplates.length : 0,
    auditLogCount: Array.isArray(state?.auditLogs) ? state.auditLogs.length : 0
  };
}

function assertValidStateSnapshot(state) {
  if (!state || typeof state !== "object") {
    throw new Error("A Passreserve state snapshot object is required.");
  }
  ...
  for (const key of requiredCollections) {
    if (!Array.isArray(state[key])) {
      throw new Error(`State snapshot is missing required array: ${key}.`);
    }
  }

  if (!state.siteSettings || !state.aboutPage) {
    throw new Error("State snapshot is missing site settings or about page content.");
  }
}
  ...
  for (const registration of state.registrations) {
    ...
    await prisma.registration.create({
      data: {
        ...
        refundPolicyAcceptedAt: toDateValue(registration.refundPolicyAcceptedAt),
...
async function rewritePrismaState(prisma, state) {
  await prisma.$transaction(async (tx) => {
    await tx.$executeRawUnsafe(`SELECT pg_advisory_xact_lock(${SYSTEM_LOCK_ID})`);
    await replacePrismaState(tx, state);
  });
}
...
function reconcileStateShape(state) {
  ...
  if (!Array.isArray(state.emailTemplates)) {
    ...
    const templateMap = new Map(
      state.emailTemplates.map((template) => [template.slug, template])
    );

    for (const template of defaultTemplates) {
      const existing = templateMap.get(template.slug);
  ...
  for (const organizer of state.organizers || []) {
    ...
    if (!("registrationQuestionnaireConfig" in organizer)) {
      organizer.registrationQuestionnaireConfig = null;
      changed = true;
    }

    if (typeof organizer.registrationLanguagePromptEnabled !== "boolean") {
  ...
  for (const event of state.events || []) {
    ...
    if (!("registrationQuestionnaireConfig" in event)) {
      event.registrationQuestionnaireConfig = null;
      changed = true;
    }

    if (!("registrationLanguagePromptEnabled" in event)) {
      event.registrationLanguagePromptEnabled = null;
      changed = true;
    }

    if (!("refundPolicyType" in event)) {
      event.refundPolicyType = String(event.cancellationPolicy || "").trim()
  ...
  for (const registration of state.registrations || []) {
    ...
    if (!("refundPolicyAcceptedAt" in registration)) {
      registration.refundPolicyAcceptedAt = null;
      changed = true;
    }

    if (!("refundPolicySnapshot" in registration)) {
      registration.refundPolicySnapshot = null;
      changed = true;
    }
...
export async function restorePrismaStateSnapshot(prisma, state) {
  assertValidStateSnapshot(state);
  const draft = structuredClone(state);
  draft.version = 1;
  reconcileStateShape(draft);
  assertValidStateSnapshot(draft);
  assertSafeToSeedDatabaseState(draft);
  await rewritePrismaState(prisma, draft);
  return summarizePersistentStateSnapshot(draft);
}
  ...
  if (getStorageMode() === "database") {
    try {
      ...
      if (!hasStateData(state)) {
        assertSafeToSeedDatabaseState(state);
        state = await buildSeedState();
        assertValidStateSnapshot(state);
        await rewritePrismaState(prisma, state);
        return state;
      }
      ...
      if (error instanceof ProductionDatabaseStateError) {
        throw error;
      }

...
import { PAYMENT_SPLIT_MODE, normalizePaymentSplitMode } from "./passreserve-domain.js";
...
export function isOccurrenceUsingOnlinePayments(entry = {}, ticketCategories = []) {
  if (normalizePaymentSplitMode(entry.paymentSplitMode) === PAYMENT_SPLIT_MODE.FIXED_AMOUNTS) {
    return ticketCategories.some(
      (category) =>
        category?.isActive !== false &&
        Math.max(0, Math.round(safeNumber(category?.fixedOnlineAmountCents, 0))) > 0
    );
  }

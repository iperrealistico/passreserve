...
export const ORGANIZER_REQUEST_RATE_LIMIT_ATTEMPTS = 4;
export const ORGANIZER_REQUEST_RATE_LIMIT_WINDOW_SECONDS = 30 * 60;
export const ORGANIZER_REQUEST_EMAIL_RATE_LIMIT_ATTEMPTS = 3;

let rateLimitWriteQueue = Promise.resolve();

async function acquireDatabaseAuthKeyLock(tx, key) {
...
async function readRawRateLimitEntries() {
  const filePath = getAuthRateLimitFilePath();
...
async function consumeDatabaseRateLimit(key, limit, windowSeconds) {
  ...
  return prisma.$transaction(async (tx) => {
    await acquireDatabaseAuthKeyLock(tx, key);
...
function buildSubmissionRateLimitKey(scope, identifier) {
  return `submission:${scope}:${identifier}`;
}

function buildSingleUseTokenKey(scope, token) {
  return `single_use:${scope}:${token}`;
}
...
async function consumeSingleUseDatabaseToken(key, windowSeconds) {
  const prisma = getPrismaClient();
  const now = new Date();
  const nextExpiry = new Date(now.getTime() + windowSeconds * 1000);
...
async function consumeSingleUseFileToken(key, windowSeconds) {
  return mutateFileRateLimits(async (entries) => {
    const now = Date.now();
    const current = entries[key];
    const expiresAt = current?.expiresAt ? new Date(current.expiresAt).getTime() : 0;
...
async function consumeSingleUseToken(key, windowSeconds) {
  if (getStorageMode() === "database") {
    try {
      return await consumeSingleUseDatabaseToken(key, windowSeconds);
    } catch (error) {
      logDatabaseFallback(
        "[passreserve-auth-security] single-use token database check failed, falling back to file storage",
        error
      );
    }
  }

  return consumeSingleUseFileToken(key, windowSeconds);
}

export async function consumeOrganizerRequestRateLimit(
  { ip, limit = ORGANIZER_REQUEST_RATE_LIMIT_ATTEMPTS, windowSeconds = ORGANIZER_REQUEST_RATE_LIMIT_WINDOW_SECONDS } = {}
) {
  const resolvedIp = ip || (await getRequestIpAddress());
  const key = buildSubmissionRateLimitKey("organizer_request", resolvedIp);
  ...
  if (getStorageMode() === "database") {
    try {
      ...
      logDatabaseFallback(
        "[passreserve-auth-security] organizer request rate limit database check failed, falling back to file rate limits",
        error
      );
    }
  }
...
export async function consumeOrganizerRequestEmailRateLimit(
  email,
  {
    limit = ORGANIZER_REQUEST_EMAIL_RATE_LIMIT_ATTEMPTS,
    windowSeconds = ORGANIZER_REQUEST_RATE_LIMIT_WINDOW_SECONDS
  } = {}
) {
  ...
  const key = buildSubmissionRateLimitKey("organizer_request_email", normalizedEmail);
  ...
  if (getStorageMode() === "database") {
    try {
      ...
      logDatabaseFallback(
        "[passreserve-auth-security] organizer request email rate limit database check failed, falling back to file rate limits",
        error
      );
    }
  }
...
export async function consumeOrganizerRequestCaptchaToken(
  token,
  { windowSeconds = ORGANIZER_REQUEST_RATE_LIMIT_WINDOW_SECONDS } = {}
) {
  if (!token) {
  ...
  const key = buildSingleUseTokenKey("organizer_request_altcha", token);
  return consumeSingleUseToken(key, windowSeconds);
}

export async function consumeOrganizerRequestSubmissionToken(
  token,
  { windowSeconds = ORGANIZER_REQUEST_RATE_LIMIT_WINDOW_SECONDS } = {}
) {
  if (!token) {
  ...
  const key = buildSingleUseTokenKey("organizer_request_submission", token);
  return consumeSingleUseToken(key, windowSeconds);
}
...
export async function pruneExpiredAuthRateLimits(now = new Date()) {
  if (getStorageMode() === "database") {
    try {
      const prisma = getPrismaClient();
      ...
      logDatabaseFallback(
        "[passreserve-auth-security] auth rate-limit prune failed in database mode, falling back to file cleanup",
        error
      );
    }
  }
  ...
  rateLimitWriteQueue = rateLimitWriteQueue
    .catch(() => undefined)
    .then(async () => {
      const rawEntries = await readRawRateLimitEntries();
      const beforeCount = Object.keys(rawEntries || {}).length;
      const nextEntries = pruneExpiredRateLimits(rawEntries, now.getTime());

      await writeRateLimitEntries(nextEntries);

      return {
        mode: "file",
        deletedCount: Math.max(0, beforeCount - Object.keys(nextEntries).length)
      };
    });

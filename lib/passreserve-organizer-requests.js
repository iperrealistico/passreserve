...
export async function submitOrganizerRequest(input) {
  ...
  if (storage.mode === "unavailable") {
    return {
      ok: false,
      message:
        "Organizer requests are temporarily unavailable in this environment. Please email contact@leonardofiori.it instead."
    };
  }
  ...
  try {
    ...
    return {
      ok: false,
      message:
        "We couldn’t save this organizer request right now. Please try again in a moment or email contact@leonardofiori.it."
    };
  }
}

...
export const phaseFoundation = {
  ...
  summary:
    "Passreserve.com keeps the event-platform vocabulary, entity boundaries, payment-state model, and transition rules aligned so the product can grow without losing clarity."
};
...
export const compatibilityAreas = [
  ...
  },
  {
    area: "Admin shells",
    detail:
      "Keep organizer-admin and platform-admin layouts, navigation patterns, and operations-first flows while swapping in event-domain data."
  },
  {
...
export const antiCorruptionRules = [
  ...
  "Do not remove legacy bridge structures until organizer, occurrence, registration, and payment flows are stable in the active product."
];
...
export const PAYMENT_SPLIT_MODE = {
  PERCENTAGE: "PERCENTAGE",
  FIXED_AMOUNTS: "FIXED_AMOUNTS"
};

export function normalizePaymentSplitMode(value) {
  return value === PAYMENT_SPLIT_MODE.FIXED_AMOUNTS
    ? PAYMENT_SPLIT_MODE.FIXED_AMOUNTS
    : PAYMENT_SPLIT_MODE.PERCENTAGE;
}
...
export function calculatePaymentBreakdown({
  ...
  const normalizedPaymentSplitMode = normalizePaymentSplitMode(paymentSplitMode);
  const hasValidFixedAmounts =
    normalizedPaymentSplitMode === PAYMENT_SPLIT_MODE.FIXED_AMOUNTS &&
    Number.isInteger(fixedOnlineAmountCents) &&
    Number.isInteger(fixedDueAtEventCents) &&
    fixedOnlineAmountCents >= 0 &&
    fixedDueAtEventCents >= 0 &&
    fixedOnlineAmountCents + fixedDueAtEventCents === unitPriceCents;
  const onlineAmountCents = hasValidFixedAmounts
    ? fixedOnlineAmountCents * safeQuantity
    : Math.round((subtotalCents * safePrepayPercentage) / 100);
  const dueAtEventCents = hasValidFixedAmounts
    ? fixedDueAtEventCents * safeQuantity
    : subtotalCents - onlineAmountCents;

  return {
    unitPrice,
    quantity: safeQuantity,
    prepayPercentage: safePrepayPercentage,
    paymentSplitMode: hasValidFixedAmounts
      ? PAYMENT_SPLIT_MODE.FIXED_AMOUNTS
      : PAYMENT_SPLIT_MODE.PERCENTAGE,
...
export const discoveryEntries = [
  {
    ...
    organizerNote:
      "The host page leads with the next three dates and an easy venue map.",
    keywords: [
      "dolomites",
      "sunrise",
      "trail",
  ...
  },
  {
    ...
    organizerNote:
      "The host page groups nearby dates into a calm agenda instead of a long scrolling list.",
    keywords: [
      "lake",
      "weekender",
      "wellness",
  ...
  },
  {
    ...
    organizerNote:
      "The host page keeps featured event types up front so city searches land on something useful right away.",
    keywords: [
      "gravel",
      "social",
      "camp",
  ...
  },
  {
    ...
    organizerNote:
      "The event listing takes over once guests want the full menu, inclusions, and policy details.",
    keywords: [
      "food",
      "workshop",
      "pasta",
  ...
  },
  {
    ...
    organizerNote:
      "The host page emphasizes seasonal series and recurring sunset dates instead of a generic catalog feel.",
    keywords: [
      "sunset",
      "breathwork",
      "lecce",
...
export const discoverySignals = [
  {
    title: "Host pages feel like destinations",
    detail:
      "Exact host matches should land on a credible page first, then open into featured event types and upcoming dates."
  },
  {
...
export const discoveryJourneys = [
  {
    ...
    steps: [
      ...
      "Open the best-fit host page, then continue to a specific event listing."
    ]
  },
  {

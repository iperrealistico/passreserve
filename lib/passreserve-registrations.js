...
function getPendingPaymentStatusCopy(payment, environment) {
  if (environment.mode === "preview") {
    return {
      ...
      headline: "Registration confirmed, payment preview ready",
      nextStep:
        "This environment does not have live Stripe keys, so Passreserve.com opens a local preview route instead of real Checkout while still keeping the registration in a payment-pending state."
    };
  }
...
function getProviderLabel(providerMode) {
  switch (providerMode) {
    ...
    case "stripe-preview":
      return "Stripe preview checkout";
    case "venue":
    default:
...
function buildDecoratedOccurrence(organizer, event, occurrence) {
  ...
  return {
    ...
    registrationStatusLabel:
      event.prepayPercentage > 0
        ? "Registration hold, confirmation, then Stripe Checkout"
        : "Registration hold then confirmation",
    ticketCategories
  };
}
...
function getPendingPaymentContext(slug, eventSlug, paymentToken) {
  ...
  if (!payload || payload.slug !== slug || payload.eventSlug !== eventSlug) {
    return {
      state: "invalid",
      title: "This payment handoff could not be verified.",
      message:
        "Open the event page again and create a fresh registration if you still need a live payment route."
    };
  }
  ...
  if (!context || !context.selectedOccurrence || !context.selectedTicketCategory) {
    return {
      ...
      message:
        "The payment token points to an occurrence that is no longer published in this sample dataset."
    };
  }
...
export const registrationFlowPhase = paymentPhase;
...
export const registrationLifecycleSignals = [
  ...
  {
    title: "Checkout after confirmation",
    detail:
      "Occurrences that require an online amount now leave the confirmation page for Stripe Checkout or an explicit preview fallback when Stripe keys are absent."
  }
];
      ...
      },
      {
        ...
        detail:
          payment.onlineAmount > 0
            ? `Confirming here opens the ${getStripeEnvironmentState().mode === "preview" ? "payment preview" : "Stripe Checkout"} step for ${payment.onlineAmountLabel} online before the registration is fully settled.`
...
export async function confirmRegistrationHold(input) {
  ...
  const statusCopy = getFinalizedStatusCopy(confirmationPayload);
  ...
  return {
    ...
    timeline: [
      ...
      },
      {
        ...
        detail:
          payload.payment.onlineAmount > 0
            ? `${paymentProvider.label} recorded ${payload.payment.onlineAmountLabel} online and the registration now sits in ${payload.registrationStatus}.`
...
export async function resumeRegistrationPayment(input) {
  ...
  if (context.state !== "ready") {
    return {
      ok: false,
      message:
        "This payment handoff is no longer available. Return to the event page and create a fresh registration if needed."
    };
  }
  ...
  if (context.paymentExpired) {
    return {
      ok: false,
      message:
        "The payment window has expired, so the registration can no longer reopen Checkout from this token."
    };
  }
  ...
  try {
    ...
    logRegistrationEvent("REGISTRATION_PAYMENT_RESUMED", {
      ...
      registrationCode: context.registrationCode,
      stripeSessionId: checkoutSession.sessionId,
      message: `Reopened the pending payment route for ${context.event.title}.`
    });
    ...
    return {
      ...
      message:
        error instanceof Error
          ? error.message
          : "The pending payment route could not be reopened."
    };
  }
}
...
export async function resolveSuccessfulRegistrationConfirmation(input) {
  ...
  if (!input.sessionId) {
    return {
      state: "invalid",
      title: "The Stripe return is missing a session identifier.",
      message:
        "Use the cancel route to reopen Checkout or return to the event page and start a new registration if needed."
    };
  }
  ...
  return {
    ...
    message:
      "The session returned before the online amount moved into a paid state. You can reopen Checkout from the pending-payment route and try again."
  };
}

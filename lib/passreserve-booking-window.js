...
export function getOrganizerBookingWindowGate(
  ...
  const skipCloseConstraint = options.skipCloseConstraint === true;
  const skipOpenConstraint = options.skipOpenConstraint === true;
  ...
  if (
    ...
    hoursUntilStart < settings.minAdvanceHours
  ) {
    return {
      ...settings,
      allowed: false,
  ...
  if (!skipOpenConstraint && settings.maxAdvanceDays && daysUntilStart > settings.maxAdvanceDays) {
    return {
      ...settings,
      allowed: false,
...
export function getRegistrationAvailabilityGate(
  ...
  const hasExplicitOpenWindow = Boolean(
    toDate(occurrence.salesWindowStartsAt) || toDate(event.salesWindowStartsAt)
  );
  const hasExplicitCloseWindow = Boolean(
    toDate(occurrence.salesWindowEndsAt) || toDate(event.salesWindowEndsAt)
  );
  ...
  const bookingWindow = getOrganizerBookingWindowGate(
    ...
    now,
    {
      skipOpenConstraint: hasExplicitOpenWindow,
      skipCloseConstraint: hasExplicitCloseWindow
    }
  );

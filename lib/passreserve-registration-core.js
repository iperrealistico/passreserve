...
export { normalizeRegistrationLocale } from "./passreserve-registration-language.js";
...
export function buildRegistrationLineItems(
  ...
  const normalizedPaymentSplitConfig =
    paymentSplitConfig && typeof paymentSplitConfig === "object"
      ? {
          paymentSplitMode: normalizePaymentSplitMode(paymentSplitConfig.paymentSplitMode),
          prepayPercentage: Number(paymentSplitConfig.prepayPercentage || 0)
        }
      : {
          paymentSplitMode: PAYMENT_SPLIT_MODE.PERCENTAGE,
          prepayPercentage: Number(paymentSplitConfig || 0)
        };
  ...
  return getSafeEntries(requestedItems).map((item, index) => {
    ...
    const payment = calculatePaymentBreakdown({
      ...
      prepayPercentage: normalizedPaymentSplitConfig.prepayPercentage,
      paymentSplitMode: normalizedPaymentSplitConfig.paymentSplitMode,
      fixedOnlineAmountCents: ticketCategory?.fixedOnlineAmountCents,
      fixedDueAtEventCents: ticketCategory?.fixedDueAtEventCents
    });
...
export function prepareRegistrationBuild({
  ...
  if (normalizePaymentSplitMode(paymentSplitMode) === PAYMENT_SPLIT_MODE.FIXED_AMOUNTS) {
    const hasInvalidFixedAmounts = requestedItems.some((item) => {
      const ticketCategory = ticketCategoryMap.get(item.ticketCategoryId);

      return (
        !Number.isInteger(ticketCategory?.fixedOnlineAmountCents) ||
        !Number.isInteger(ticketCategory?.fixedDueAtEventCents) ||
        ticketCategory.fixedOnlineAmountCents < 0 ||
        ticketCategory.fixedDueAtEventCents < 0 ||
        ticketCategory.fixedOnlineAmountCents + ticketCategory.fixedDueAtEventCents !==
          ticketCategory.unitPriceCents
      );
    });

    if (hasInvalidFixedAmounts) {
      return {
        ok: false,
        message: "This event's payment amounts need to be corrected before registration can continue.",
        fieldErrors: {
          items: "Refresh the page or contact the event organizer."
        }
      };
    }
  }

...
function createLocalizedTemplateField(en, it) {
  return {
    en,
    it
  };
...
export function createDefaultEmailTemplates() {
  return [
    ...
    {
      id: "email-organizer-access-invitation",
      slug: "organizer_access_invitation",
      audience: "Organizer admin",
      category: "Host onboarding",
      subject: "Your Passreserve organizer access is ready",
      trigger: "A valid organizer application is provisioned automatically",
      preview:
        "Sends the organizer admin login path, one-time reset link, and the private/public page state.",
      placeholders: [
        "{{organizer_name}}",
        "{{admin_name}}",
        "{{admin_email}}",
      ...
      bodyHtml:
        "<p>Hi {{admin_name}},</p><p>Your Passreserve organizer account for <strong>{{organizer_name}}</strong> is ready.</p><p><strong>Login:</strong> <a href=\"{{login_url}}\">{{login_url}}</a></p><p><strong>Set your first password:</strong> <a href=\"{{reset_url}}\">{{reset_url}}</a></p><p>Your public page slug is currently <strong>{{public_slug}}</strong> and the page starts private until you publish it from organizer settings.</p>",
      createdAt: "2026-04-27T17:30:00.000Z",
      updatedAt: "2026-04-27T17:30:00.000Z"
    },
    {
      id: "email-organizer-application-alert",
      slug: "organizer_application_alert",
      audience: "Platform admin",
      category: "Host onboarding",
      subject: "Organizer application processed for {{organizer_name}}",
      trigger: "An organizer application is stored, duplicated, provisioned, or needs email follow-up",
      preview:
        "Keeps the platform owner updated on provisioning status without leaving the shared admin console.",
      placeholders: [
        "{{organizer_name}}",
        "{{contact_name}}",
        "{{contact_email}}",
      ...
      bodyHtml:
        "<p>A Passreserve organizer application was processed.</p><p><strong>Organizer:</strong> {{organizer_name}}</p><p><strong>Contact:</strong> {{contact_name}} ({{contact_email}})</p><p><strong>City:</strong> {{city}}</p><p><strong>Event focus:</strong> {{event_focus}}</p><p><strong>Status:</strong> {{provisioning_status}}</p><p><strong>Public slug:</strong> {{public_slug}}</p>",
      createdAt: "2026-04-27T17:30:00.000Z",
      updatedAt: "2026-04-27T17:30:00.000Z"
    },
    {
      ...
      subjectTranslations: createLocalizedTemplateField(
        "Confirm your Passreserve registration",
        "Conferma la tua registrazione Passreserve"
      ),
      trigger: "A registration hold is created and still needs attendee confirmation",
      preview:
        "Invites the attendee to confirm the hold, repeats the payment split, and can explain when the organizer prepared the registration.",
      previewTranslations: createLocalizedTemplateField(
        "Invites the attendee to confirm the hold, repeats the payment split, and can explain when the organizer prepared the registration.",
        "Invita il partecipante a confermare la hold, ripete la ripartizione del pagamento e può spiegare quando la registrazione è stata preparata dall'organizer."
      ),
      placeholders: [
        "{{event_name}}",
        "{{occurrence_label}}",
        ...
        "{{online_amount}}",
        "{{due_at_event}}",
        "{{registration_source_note}}",
        "{{registration_source_label}}",
        "{{registration_origin_label}}"
      ],
      bodyHtml:
        "<p>Hi {{attendee_name}},</p><p>{{registration_source_note}}</p><p>Your place for <strong>{{event_name}}</strong> on {{occurrence_label}} is being held briefly.</p><p>Confirm here: <a href=\"{{confirmation_url}}\">{{confirmation_url}}</a></p><p>Online now: {{online_amount}}. Due at the event: {{due_at_event}}.</p>",
      bodyHtmlTranslations: createLocalizedTemplateField(
        "<p>Hi {{attendee_name}},</p><p>{{registration_source_note}}</p><p>Your place for <strong>{{event_name}}</strong> on {{occurrence_label}} is being held briefly.</p><p>Confirm here: <a href=\"{{confirmation_url}}\">{{confirmation_url}}</a></p><p>Online now: {{online_amount}}. Due at the event: {{due_at_event}}.</p>",
        "<p>Ciao {{attendee_name}},</p><p>{{registration_source_note}}</p><p>Il tuo posto per <strong>{{event_name}}</strong> del {{occurrence_label}} è stato riservato per poco tempo.</p><p>Conferma qui: <a href=\"{{confirmation_url}}\">{{confirmation_url}}</a></p><p>Da pagare online ora: {{online_amount}}. Da pagare all'evento: {{due_at_event}}.</p>"
      ),
      createdAt: "2026-04-01T09:00:00.000Z",
      updatedAt: "2026-04-01T09:00:00.000Z"
    },
    {
      ...
      subjectTranslations: createLocalizedTemplateField(
        "Your Passreserve registration is confirmed",
        "La tua registrazione Passreserve è confermata"
      ),
      ...
      previewTranslations: createLocalizedTemplateField(
        "Shares the registration code, event summary, and arrival guidance.",
        "Condivide il codice registrazione, il riepilogo dell'evento e le indicazioni per l'arrivo."
      ),
      placeholders: [
        "{{registration_code}}",
        "{{event_name}}",
      ...
      bodyHtml:
        "<p>{{registration_source_note}}</p><p>Your registration <strong>{{registration_code}}</strong> for {{event_name}} is confirmed.</p><p>Venue: {{venue_name}}</p><p>Amount still due at the event: {{due_at_event}}</p>",
      bodyHtmlTranslations: createLocalizedTemplateField(
        "<p>{{registration_source_note}}</p><p>Your registration <strong>{{registration_code}}</strong> for {{event_name}} is confirmed.</p><p>Venue: {{venue_name}}</p><p>Amount still due at the event: {{due_at_event}}</p>",
        "<p>{{registration_source_note}}</p><p>La tua registrazione <strong>{{registration_code}}</strong> per {{event_name}} è confermata.</p><p>Luogo: {{venue_name}}</p><p>Importo ancora dovuto all'evento: {{due_at_event}}</p>"
      ),
      createdAt: "2026-04-01T09:00:00.000Z",
      updatedAt: "2026-04-01T09:00:00.000Z"
    },
    {
      id: "email-attendee-payment-requested",
      slug: "attendee_payment_requested",
      audience: "Attendee",
      category: "Payments",
      subject: "Complete payment for your Passreserve registration",
      subjectTranslations: createLocalizedTemplateField(
        "Complete payment for your Passreserve registration",
        "Completa il pagamento per la tua registrazione Passreserve"
      ),
      trigger:
      ...
      previewTranslations: createLocalizedTemplateField(
        "Explains that the organizer already prepared the registration, then leads with the payment link and the online-versus-venue split.",
        "Spiega che l'organizer ha già preparato la registrazione e mette in evidenza il link di pagamento e la ripartizione tra online e saldo all'evento."
      ),
      placeholders: [
        "{{attendee_name}}",
        "{{registration_code}}",
    ...
    },
    {
      ...
      subjectTranslations: createLocalizedTemplateField(
        ...
        "La tua registrazione Passreserve è stata annullata"
      ),
      ...
      previewTranslations: createLocalizedTemplateField(
        "Confirms the cancellation clearly and explains any payment or refund state in plain language.",
        "Conferma chiaramente l'annullamento e spiega lo stato di pagamento o rimborso in modo semplice."
      ),
      placeholders: [
        "{{registration_code}}",
        "{{event_name}}",
      ...
      bodyHtmlTranslations: createLocalizedTemplateField(
        "<p>Your registration <strong>{{registration_code}}</strong> for {{event_name}} on {{occurrence_label}} has been cancelled.</p><p>{{refund_state}}</p><p>If you need help or want to discuss a replacement date, reply to {{support_reply_email}}.</p>",
        "<p>La tua registrazione <strong>{{registration_code}}</strong> per {{event_name}} del {{occurrence_label}} è stata annullata.</p><p>{{refund_state}}</p><p>Se hai bisogno di aiuto o vuoi valutare una data sostitutiva, rispondi a {{support_reply_email}}.</p>"
      ),
      createdAt: "2026-04-01T09:00:00.000Z",
      updatedAt: "2026-04-01T09:00:00.000Z"
    },
    {
      ...
      subjectTranslations: createLocalizedTemplateField(
        "Payment received for your Passreserve registration",
        "Pagamento ricevuto per la tua registrazione Passreserve"
      ),
      ...
      previewTranslations: createLocalizedTemplateField(
        "Confirms the online amount and repeats any remaining venue balance.",
        "Conferma l'importo pagato online e ripete l'eventuale saldo rimanente all'evento."
      ),
      placeholders: [
        "{{registration_code}}",
        "{{paid_online}}",
      ...
      bodyHtmlTranslations: createLocalizedTemplateField(
        "<p>Payment received for {{event_name}}.</p><p>Registration: {{registration_code}}</p><p>Paid online: {{paid_online}}</p><p>Due at the event: {{due_at_event}}</p>",
        "<p>Pagamento ricevuto per {{event_name}}.</p><p>Registrazione: {{registration_code}}</p><p>Pagato online: {{paid_online}}</p><p>Da pagare all'evento: {{due_at_event}}</p>"
      ),
      createdAt: "2026-04-01T09:00:00.000Z",
      updatedAt: "2026-04-01T09:00:00.000Z"
    },
    {
      ...
      previewTranslations: createLocalizedTemplateField(
        "Reminds the guest about the date, time, venue, and anything still due at the event.",
        "Ricorda al partecipante data, orario, venue e qualsiasi importo ancora dovuto all'evento."
      ),
      placeholders: [
        "{{attendee_name}}",
        "{{event_name}}",
    ...
    },
    {
      ...
      previewTranslations: createLocalizedTemplateField(
        "Explains that a specific event date has been cancelled and repeats the payment or refund state clearly.",
        "Spiega che una specifica data dell'evento è stata annullata e ripete chiaramente lo stato del pagamento o del rimborso."
      ),
      placeholders: [
        "{{event_name}}",
        "{{occurrence_label}}",
      ...
      bodyHtmlTranslations: createLocalizedTemplateField(
        "<p>The scheduled date for <strong>{{event_name}}</strong> on {{occurrence_label}} has been cancelled.</p><p>{{refund_state}}</p><p>Please reply to {{support_reply_email}} if you need help with next steps.</p>",
        "<p>La data prevista per <strong>{{event_name}}</strong> del {{occurrence_label}} è stata annullata.</p><p>{{refund_state}}</p><p>Rispondi a {{support_reply_email}} se hai bisogno di aiuto per i prossimi passi.</p>"
      ),
      createdAt: "2026-04-01T09:00:00.000Z",
      updatedAt: "2026-04-01T09:00:00.000Z"
    },
...
export function createSiteSettings() {
  return {
    ...
    launchInbox: "contact@leonardofiori.it",
...
function buildEventRecord(organizerRecord, event) {
  ...
  return {
    ...
    refundPolicyType:
      event.policies?.length || event.cancellationPolicy
        ? "REFUNDABLE_WITH_CONDITIONS"
        : null,
    collectDietaryInfo: event.collectDietaryInfo !== false,
    registrationQuestionnaireConfig: null,
    registrationLanguagePromptEnabled: null,
    registrationConfirmationMode: null,
